 ...
 *
 * At the moment, it's a sample to validate the automatic test environment, in
 * the scope of instrumented unit tests.
 *
 * Don't take it as an example of completeness.
 *
 * See
 * http://developer.android.com/intl/es/training/testing/unit-testing/instrumented-unit-tests.html
 * .
 */
@SmallTest
public class OCFileUnitTest {
  ...
  private static final long MODIFICATION_TIMESTAMP_AT_LAST_SYNC_FOR_DATA =
      6543210987L;
  ...
  public void writeThenReadAsParcelable() {
    ...
    mFile.setModificationTimestampAtLastSyncForData(
    ...
    mFile.setAvailableOfflineStatus(
    ...
    assertThat(fileReadFromParcel.getCreationTimestamp(),
               is(CREATION_TIMESTAMP));
    assertThat(fileReadFromParcel.getModificationTimestamp(),
               is(MODIFICATION_TIMESTAMP));
    assertThat(fileReadFromParcel.getModificationTimestampAtLastSyncForData(),
               is(MODIFICATION_TIMESTAMP_AT_LAST_SYNC_FOR_DATA));
    assertThat(fileReadFromParcel.getLastSyncDateForProperties(),
               is(LAST_SYNC_DATE_FOR_PROPERTIES));
    assertThat(fileReadFromParcel.getLastSyncDateForData(),
               is(LAST_SYNC_DATE_FOR_DATA));
    assertThat(fileReadFromParcel.getAvailableOfflineStatus(),
