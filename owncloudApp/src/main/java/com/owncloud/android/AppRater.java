...
private static final String APP_RATER_PREF_DATE_FIRST_LAUNCH =
	"date_first_launch";
...
public static void appLaunched(final Context mContext,
	...
	SharedPreferences prefs =
		mContext.getSharedPreferences(APP_RATER_PREF_TITLE, 0);
	...
	if (dateFirstLaunch == 0) {
		...
		Timber.d("The app has been launched in " + dateFirstLaunch +
		         " for the first time");
	...
	if (launchCount >= LAUNCHES_UNTIL_PROMPT) {
		...
		Timber.d("The date of the first launch + days until prompt is " +
		         dateFirstLaunch + daysToMilliseconds(DAYS_UNTIL_PROMPT));
		Timber.d("The date of the neutral click + days until neutral click is " +
		         dateNeutralClick + daysToMilliseconds(DAYS_UNTIL_NEUTRAL_CLICK));
		if (System.currentTimeMillis() >=
		    Math.max(dateFirstLaunch + daysToMilliseconds(DAYS_UNTIL_PROMPT),
		             dateNeutralClick +
		             daysToMilliseconds(DAYS_UNTIL_NEUTRAL_CLICK))) {
			Timber.d(
				"The current moment is later than any of the days set, so let's show the rate dialog");
