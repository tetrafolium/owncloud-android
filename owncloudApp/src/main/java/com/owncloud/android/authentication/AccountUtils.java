 ...
 * @return The ownCloud {@link Account} currently saved in preferences, or the
 *     first
 *                      {@link Account} available, if valid (still registered
 * in the system as ownCloud account). If none is available and valid, returns
 * null.
 */
...
public static boolean exists(final String accountName,
...
/**
 * Returns owncloud account identified by accountName or null if it does not
 * exist.
 * @param context
 * @param accountName name of account to be returned
 * @return owncloud account named accountName
 */
public static Account getOwnCloudAccountByName(final Context context,
...
public static boolean setCurrentOwnCloudAccount(final Context context,
                                                final String accountName) {
...
/**
 * Update the accounts in AccountManager to meet the current version of
 * accounts expected by the app, if needed.
 *
 * Introduced to handle a change in the structure of stored account names
 * needed to allow different OC servers in the same domain, but not in the
 * same path.
 *
...
public static void updateAccountVersion(final Context context) {
	...
	if (currentAccount != null) {
		String currentAccountVersion = accountMgr.getUserData(
			currentAccount, Constants.KEY_OC_ACCOUNT_VERSION);

		if (currentAccountVersion == null) {
			Timber.i("Upgrading accounts to account version #%s", ACCOUNT_VERSION);
			Account[] ocAccounts =
				accountMgr.getAccountsByType(MainApp.Companion.getAccountType());
			String serverUrl, username, newAccountName, password;
			Account newAccount;
			for (Account account : ocAccounts) {
				// build new account name
				serverUrl =
					accountMgr.getUserData(account, Constants.KEY_OC_BASE_URL);
				...
				if (!newAccountName.equals(account.name)) {
					...
					accountMgr.addAccountExplicitly(
						newAccount, (password != null) ? password : "", null);

					// copy base URL
					accountMgr.setUserData(newAccount, Constants.KEY_OC_BASE_URL,
					                       serverUrl);

					// copy server version
					accountMgr.setUserData(
						newAccount, Constants.KEY_OC_VERSION,
						accountMgr.getUserData(account, Constants.KEY_OC_VERSION));

					// copy cookies
					accountMgr.setUserData(
						newAccount, Constants.KEY_COOKIES,
						accountMgr.getUserData(account, Constants.KEY_COOKIES));

					String isOauthStr =
						accountMgr.getUserData(account, Constants.KEY_SUPPORTS_OAUTH2);
					boolean isOAuth = OAUTH_SUPPORTED_TRUE.equals(isOauthStr);
					if (isOAuth) {
						accountMgr.setUserData(newAccount, Constants.KEY_SUPPORTS_OAUTH2,
						                       OAUTH_SUPPORTED_TRUE);
					}
					...
					// servers which base URL is in the root of their domain need no
					// change
				...
				Timber.d("Setting version " + ACCOUNT_VERSION + " to " +
				         newAccountName);
...
/**
 * Access the version of the OC server corresponding to an account SAVED IN
 * THE ACCOUNTMANAGER
 *
 * @param   account     ownCloud account
 * @return Version of the OC server corresponding to account, according to the
 *     data saved
	...
	if (account != null) {
		...
		FileDataStorageManager fileDataStorageManager =
			new FileDataStorageManager(
				MainApp.Companion.getAppContext(), account,
				MainApp.Companion.getAppContext().getContentResolver());
		OCCapability capability =
			fileDataStorageManager.getCapability(account.name);
		if (capability != null) {
