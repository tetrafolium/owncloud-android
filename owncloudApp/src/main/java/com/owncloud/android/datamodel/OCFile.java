...
public class OCFile implements Parcelable, Comparable<OCFile> {
  ...
  public static final Parcelable.Creator<OCFile> CREATOR =
      new Parcelable.Creator<OCFile>() {
        @Override
  ...
  private final static String PERMISSION_SHARED_WITH_ME =
  ...
  private String mEtagInConflict; // Save file etag in the server, when there is
                                  // a conflict. No conflict =  null
  ...
  /**
   * URI to the local path of the file contents, if stored in the device; cached
   * after first call to {@link #getStorageUri()}
   */
  private Uri mLocalUri;

  /**
   * Exportable URI to the local path of the file contents, if stored in the
   * device.
   *
  ...
  private OCFile(final Parcel source) {
    ...
    try {
      mAvailableOfflineStatus =
          AvailableOfflineStatus.valueOf(source.readString());
   ...
   * @return the file ID
   */
  public long getFileId() { return mId; }
  ...
  public boolean isFolder() {
    return mMimeType != null &&
        (mMimeType.equals("DIR") || mMimeType.equals("httpd/unix-directory"));
  }
   ...
   */
  public String getStoragePath() { return mLocalPath; }

  /**
   * The URI to the file contents, if stored locally
   *
   * @return A URI to the local copy of the file, or NULL if not stored in the
   *     device
   */
  ...
  public Uri getExposedFileUri(final Context context) {
    ...
    if (mExposedFileUri == null) {
      if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
        // TODO - use FileProvider with any Android version, with deeper testing
        // -> 2.2.0
        mExposedFileUri = Uri.parse(ContentResolver.SCHEME_FILE + "://" +
                                    WebdavUtils.encodePath(mLocalPath));
      } else {
        // Use the FileProvider to get a content URI
        try {
          mExposedFileUri = FileProvider.getUriForFile(
              context, context.getString(R.string.file_provider_authority),
              new File(mLocalPath));
   ...
   */
  public long getCreationTimestamp() { return mCreationTimestamp; }
   ...
   */
  public long getModificationTimestamp() { return mModifiedTimestamp; }

  /**
   * Set a UNIX timestamp of the time the time the file was modified.
   *
   * To update with the value returned by the server in every synchronization of
   ...
   */
  public void
  setModificationTimestampAtLastSyncForData(final long modificationTimestamp) {
  ...
  public void setFileName(final String name) {
    ...
    if (name != null && name.length() > 0 && !name.contains(PATH_SEPARATOR) &&
      ...
      parent =
          (parent.endsWith(PATH_SEPARATOR)) ? parent : parent + PATH_SEPARATOR;
   ...
   */
  public void setFileId(final long file_id) { mId = file_id; }
   ...
   */
  public void setParentId(final long parent_id) { mParentId = parent_id; }
   ...
   */
  public void setFileLength(final long file_len) { mLength = file_len; }
   ...
   */
  public long getFileLength() { return mLength; }
  ...
  public boolean needsUpdateThumbnail() { return mNeedsUpdateThumbnail; }
  ...
  public long getLastSyncDateForData() { return mLastSyncDateForData; }
  ...
  public void
  setAvailableOfflineStatus(final AvailableOfflineStatus availableOffline) {
  ...
  public boolean isAvailableOffline() {
    return (mAvailableOfflineStatus !=
            AvailableOfflineStatus.NOT_AVAILABLE_OFFLINE);
  }
  ...
  public int compareTo(final @NonNull OCFile another) {
    if (isFolder() && another.isFolder()) {
      return getRemotePath().toLowerCase().compareTo(
          another.getRemotePath().toLowerCase());
    } else if (isFolder()) {
      return -1;
  ...
  public String toString() {
    String asString =
        "[id=%s, name=%s,  etag=%s, tree_etag=%s, mime=%s, downloaded=%s, local=%s, remote=%s, "
        + "parentId=%s, favorite=%s]";
    asString = String.format(asString, mId, getFileName(), mEtag, mTreeEtag,
                             mMimeType, isDown(), mLocalPath, mRemotePath,
                             mParentId, mAvailableOfflineStatus);
    return asString;
  }

  public String getEtag() { return mEtag; }

  public void setEtag(final String etag) { mEtag = (etag != null ? etag : ""); }

  public String getTreeEtag() { return mTreeEtag; }

  public void setTreeEtag(final String etag) {
    mTreeEtag = (etag != null ? etag : "");
  }

  public boolean isSharedViaLink() { return mSharedByLink; }
   ...
   */
  public boolean isAudio() { return isOfType("audio/"); }

  /**
   * @return 'True' if the file contains video
   */
  public boolean isVideo() { return isOfType("video/"); }

  /**
   * @return 'True' if the file contains an image
   */
  public boolean isImage() { return isOfType("image/"); }

  /**
   * @return 'True' if the file is simple text (e.g. not application-dependent,
   *     like .doc or .docx)
   */
  public boolean isText() { return isOfType("text/"); }

  /**
   * @param   type        Type to match in the file MIME type; it's MUST include
   *     the trailing "/"
   * @return              'True' if the file MIME type matches the received
   *     parameter in the type part.
   */
   ...
   */
  public boolean isHidden() { return getFileName().startsWith("."); }
  ...
  public void setRemoteId(final String remoteId) { mRemoteId = remoteId; }
  ...
  public String getEtagInConflict() { return mEtagInConflict; }
  ...
  public boolean isSharedWithSharee() { return mSharedWithSharee; }
  ...
  public boolean isSharedWithMe() {
    ...
    return (permissions != null &&
            permissions.contains(PERMISSION_SHARED_WITH_ME));
  }
