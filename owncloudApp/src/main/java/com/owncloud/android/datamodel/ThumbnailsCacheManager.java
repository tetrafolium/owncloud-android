...
public static Bitmap mDefaultImg = BitmapFactory.decodeResource(
	...
	synchronized (mThumbnailsDiskCacheLock) {
		...
		if (mThumbnailCache == null) {
			try {
				...
				final String cachePath = MainApp.Companion.getAppContext()
				                         .getExternalCacheDir()
				                         .getPath() +
				...
				mThumbnailCache =
					new DiskLruImageCache(diskCacheDir, DISK_CACHE_SIZE,
...
public static class ThumbnailGenerationTask
	...
	try {
		if (mAccount != null) {
			OwnCloudAccount ocAccount =
				new OwnCloudAccount(mAccount, MainApp.Companion.getAppContext());
			mClient = SingleSessionManager.getDefaultSingleton().getClientFor(
				ocAccount, MainApp.Companion.getAppContext());
		}
 ...
 * @return Bitmap
 */
private Bitmap addThumbnailToCache(final String imageKey,
...
private Bitmap doOCFileInBackground() {
	...
	if (thumbnail == null || file.needsUpdateThumbnail()) {
		...
		if (file.isDown()) {
			...
			OwnCloudVersion serverOCVersion =
				AccountUtils.getServerVersion(mAccount);
			if (mClient != null && serverOCVersion != null) {
				GetMethod get;
				try {
					String uri = mClient.getBaseUri() + ""
					             + "/index.php/apps/files/api/v1/thumbnail/" + px +
					             "/" + px + Uri.encode(file.getRemotePath(), "/");
					Timber.d("URI: %s", uri);
...
private Bitmap handlePNG(final Bitmap bitmap, final int px) {
	...
	c.drawColor(ContextCompat.getColor(MainApp.Companion.getAppContext(),
...
/**
 * Show the avatar corresponding to the received account in an {@link
 * ImageView} ir {@link MenuItem}. <p> The avatar is loaded if available in
 * the cache and bound to the received UI element. The avatar is not fetched
 * from the server if not available, unless the parameter 'fetchFromServer' is
 * set to 'true'. <p> If there is no avatar stored and cannot be fetched, a
 * colored icon is generated with the first letter of the account username.
 * <p>
...
/**
 * Builds an instance to show the avatar corresponding to the received
 * account in an {@link ImageView}.
 *
 * @param imageView       The {@link ImageView} to bind the avatar to.
 * @param account         OC account which avatar will be shown.
 * @param displayRadius   The radius of the circle where the avatar will be
 *     clipped into.
 * @param fetchFromServer When 'true', if there is no avatar stored in the
 *     cache, it's fetched from
 *                        the server. When 'false', server is not accessed,
 * the fallback avatar is generated instead. USE WITH CARE, probably to be
 * removed in the future.
 */
public GetAvatarTask(final ImageView imageView, final Account account,
 ...
 * removed in the future.
 */
public GetAvatarTask(final MenuItem menuItem, final Account account,
                     final float displayRadius,
                     final boolean fetchFromServer) {
	if (account == null) {
		throw new IllegalArgumentException("Received NULL account");
...
private Drawable doAvatarInBackground() {
	...
	if (avatarBitmap != null) {
		avatarDrawable = BitmapUtils.bitmapToCircularBitmapDrawable(
			MainApp.Companion.getAppContext().getResources(), avatarBitmap);
		...
		if (mFetchFromServer) {
			...
			if (mClient != null && serverOCVersion != null) {
				...
				try {
					String uri = mClient.getBaseUri() + ""
					             + "/index.php/avatar/" +
					             AccountUtils.getUsernameOfAccount(mUsername) + "/" +
					             px;
					Timber.d("URI: %s", uri);
					get = new GetMethod(new URL(uri));
					int status = mClient.executeHttpMethod(get);
		...
		if (avatarBitmap != null) {
			...
			try {
				avatarDrawable = DefaultAvatarTextDrawable.createAvatar(
					mUsername, mDisplayRadius);
...
public static String addAvatarToCache(final String accountName,
                                      final byte[] avatarData,
                                      final int dimension) {
	final String imageKey = "a_" + accountName;

	Bitmap bitmap =
		BitmapFactory.decodeByteArray(avatarData, 0, avatarData.length);
...
cancelPotentialThumbnailWork(final Object file, final ImageView imageView) {
	final ThumbnailGenerationTask bitmapWorkerTask =
		getBitmapWorkerTask(imageView);
...
private static ThumbnailGenerationTask
getBitmapWorkerTask(final ImageView imageView) {
	if (imageView != null) {
		final Drawable drawable = imageView.getDrawable();
		if (drawable instanceof AsyncThumbnailDrawable) {
			final AsyncThumbnailDrawable asyncDrawable =
				(AsyncThumbnailDrawable)drawable;
...
private final WeakReference<ThumbnailGenerationTask>
bitmapWorkerTaskReference;

public AsyncThumbnailDrawable(final Resources res, final Bitmap bitmap,
                              final ThumbnailGenerationTask
                              bitmapWorkerTask) {
