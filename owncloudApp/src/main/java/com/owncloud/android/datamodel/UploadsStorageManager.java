...
public class UploadsStorageManager extends Observable {
  ...
  public enum UploadStatus {
    ...
    UploadStatus(final int value) { this.value = value; }
  ...
  public long storeUpload(final OCUpload ocUpload) {
    Timber.v("Inserting " + ocUpload.getLocalPath() +
             " with status=" + ocUpload.getUploadStatus());
    ...
    cv.put(ProviderTableMeta.UPLOADS_LOCAL_BEHAVIOUR,
           ocUpload.getLocalAction());
    cv.put(ProviderTableMeta.UPLOADS_FORCE_OVERWRITE,
           ocUpload.isForceOverwrite() ? 1 : 0);
    cv.put(ProviderTableMeta.UPLOADS_IS_CREATE_REMOTE_FOLDER,
           ocUpload.createsRemoteFolder() ? 1 : 0);
    cv.put(ProviderTableMeta.UPLOADS_LAST_RESULT,
           ocUpload.getLastResult().getValue());
    ...
    Timber.d("storeUpload returns with: " + result +
             " for file: " + ocUpload.getLocalPath());
    if (result == null) {
      Timber.e("Failed to insert item " + ocUpload.getLocalPath() +
               " into upload db.");
      return -1;
    } else {
  ...
  public int updateUpload(final OCUpload ocUpload) {
    Timber.v("Updating " + ocUpload.getLocalPath() +
             " with status=" + ocUpload.getUploadStatus());
    ...
    cv.put(ProviderTableMeta.UPLOADS_LAST_RESULT,
           ocUpload.getLastResult().getValue());
    cv.put(ProviderTableMeta.UPLOADS_UPLOAD_END_TIMESTAMP,
           ocUpload.getUploadEndTimestamp());
    cv.put(ProviderTableMeta.UPLOADS_TRANSFER_ID, ocUpload.getTransferId());

    int result = getDB().update(
        ProviderTableMeta.CONTENT_URI_UPLOADS, cv, ProviderTableMeta._ID + "=?",
        new String[] {String.valueOf(ocUpload.getUploadId())});

    Timber.d("updateCameraUploadSync returns with: " + result +
             " for file: " + ocUpload.getLocalPath());
    if (result != 1) {
      Timber.e("Failed to update item " + ocUpload.getLocalPath() +
               " into upload db.");
    } else {
      notifyObserversNow();
    }
  ...
  private int updateUploadInternal(final Cursor c, final UploadStatus status,
    ...
    while (c.moveToNext()) {
      ...
      String path =
          c.getString(c.getColumnIndex(ProviderTableMeta.UPLOADS_LOCAL_PATH));
      Timber.v("Updating " + path + " with status:" + status +
               " and result:" + (result == null ? "null" : result.toString()) +
               " (old:" + upload.toFormattedString() + ")");
   ...
   */
  public int updateUploadStatus(final long id, final UploadStatus status,
                                final UploadResult result,
                                final String remotePath,
                                final String localPath) {
    int returnValue = 0;
    Cursor c = getDB().query(ProviderTableMeta.CONTENT_URI_UPLOADS, null,
                             ProviderTableMeta._ID + "=?",
                             new String[] {String.valueOf(id)}, null);

    if (c.getCount() != 1) {
      Timber.e(
          c.getCount() + " items for id=" + id +
  ...
  /**
   * Remove an upload from the uploads list, known its target account and remote
   * path.
   *
  ...
  public int removeUpload(final OCUpload upload) {
    int result = getDB().delete(
        ProviderTableMeta.CONTENT_URI_UPLOADS, ProviderTableMeta._ID + "=?",
   ...
   * @param remotePath  Absolute path in the OC account target of the upload to
   *     remove.
  ...
  public int removeUploads(final String accountName) {
    int result = getDB().delete(ProviderTableMeta.CONTENT_URI_UPLOADS,
  ...
  public OCUpload getLastUploadFor(final OCFile file,
                                   final String accountName) {
    OCUpload[] uploads =
        getUploads(ProviderTableMeta.UPLOADS_REMOTE_PATH + "== ? AND " +
                       ProviderTableMeta.UPLOADS_ACCOUNT_NAME + "== ?",
                   new String[] {file.getRemotePath(), accountName},
                   ProviderTableMeta.UPLOADS_UPLOAD_END_TIMESTAMP + " desc");
    return (uploads.length > 0 ? uploads[0] : null);
  }

  private OCUpload[] getUploads(final String selection,
  ...
  private OCUpload createOCUploadFromCursor(final Cursor c) {
    ...
    if (c != null) {
      String localPath =
          c.getString(c.getColumnIndex(ProviderTableMeta.UPLOADS_LOCAL_PATH));
      String remotePath =
          c.getString(c.getColumnIndex(ProviderTableMeta.UPLOADS_REMOTE_PATH));
      String accountName =
          c.getString(c.getColumnIndex(ProviderTableMeta.UPLOADS_ACCOUNT_NAME));
      upload = new OCUpload(localPath, remotePath, accountName);

      upload.setFileSize(
          c.getLong(c.getColumnIndex(ProviderTableMeta.UPLOADS_FILE_SIZE)));
      upload.setUploadId(c.getLong(c.getColumnIndex(ProviderTableMeta._ID)));
      upload.setUploadStatus(UploadStatus.fromValue(
          c.getInt(c.getColumnIndex(ProviderTableMeta.UPLOADS_STATUS))));
      upload.setLocalAction(c.getInt(
          c.getColumnIndex((ProviderTableMeta.UPLOADS_LOCAL_BEHAVIOUR))));
      upload.setForceOverwrite(
          c.getInt(c.getColumnIndex(
              ProviderTableMeta.UPLOADS_FORCE_OVERWRITE)) == 1);
      upload.setCreateRemoteFolder(
          c.getInt(c.getColumnIndex(
              ProviderTableMeta.UPLOADS_IS_CREATE_REMOTE_FOLDER)) == 1);
      upload.setUploadEndTimestamp(c.getLong(
          c.getColumnIndex(ProviderTableMeta.UPLOADS_UPLOAD_END_TIMESTAMP)));
      upload.setLastResult(UploadResult.fromValue(
          c.getInt(c.getColumnIndex(ProviderTableMeta.UPLOADS_LAST_RESULT))));
      upload.setCreatedBy(
          c.getInt(c.getColumnIndex(ProviderTableMeta.UPLOADS_CREATED_BY)));
      upload.setTransferId(
          c.getString(c.getColumnIndex(ProviderTableMeta.UPLOADS_TRANSFER_ID)));
    }
    return upload;
  }

  /**
   * Get all uploads which are currently being uploaded or waiting in the queue
   * to be uploaded.
   */
  public OCUpload[] getCurrentAndPendingUploads() {
    ...
    return getUploads(
        ...
        new String[] {String.valueOf(UploadStatus.UPLOAD_IN_PROGRESS.value),
  ...
  public OCUpload[] getFailedUploads() {
    return getUploads(
        ...
        new String[] {String.valueOf(UploadStatus.UPLOAD_FAILED.value)}, null);
  }
  ...
  /**
   * Get all failed uploads, except for those that were not performed due to
   * lack of Wifi connection
   *
   * @return Array of failed uploads, except for those that were not performed
   *     due to lack of Wifi connection.
   */
  public OCUpload[] getFailedButNotDelayedForWifiUploads() {
    return getUploads(
        ProviderTableMeta.UPLOADS_STATUS + "== ?" +
            UploadStatus.UPLOAD_FAILED.value + " AND " +
            ProviderTableMeta.UPLOADS_LAST_RESULT + "<> ?",
   ...
   */
  public void
  updateDatabaseUploadResult(final RemoteOperationResult uploadResult,
                             final UploadFileOperation uploadFileOperation) {
    // result: success or fail notification
    Timber.d("updateDataseUploadResult uploadResult: " + uploadResult +
             " upload: " + uploadFileOperation);

    if (uploadResult.isCancelled()) {
      removeUpload(uploadFileOperation.getAccount().name,
                   uploadFileOperation.getRemotePath());
    } else {
      String localPath = (FileUploader.LOCAL_BEHAVIOUR_MOVE ==
                          uploadFileOperation.getLocalBehaviour())
                             ? uploadFileOperation.getStoragePath()
                             : null;

      if (uploadResult.isSuccess()) {
        updateUploadStatus(uploadFileOperation.getOCUploadId(),
                           UploadStatus.UPLOAD_SUCCEEDED, UploadResult.UPLOADED,
                           uploadFileOperation.getRemotePath(), localPath);
      } else {
        updateUploadStatus(uploadFileOperation.getOCUploadId(),
   ...
   */
  public void
  updateDatabaseUploadStart(final UploadFileOperation uploadFileOperation) {
    String localPath = (FileUploader.LOCAL_BEHAVIOUR_MOVE ==
                        uploadFileOperation.getLocalBehaviour())
                           ? uploadFileOperation.getStoragePath()
                           : null;
    ...
    updateUploadStatus(uploadFileOperation.getOCUploadId(),
                       UploadStatus.UPLOAD_IN_PROGRESS, UploadResult.UNKNOWN,
                       uploadFileOperation.getRemotePath(), localPath);
  }

  /**
   * Changes the status of any in progress upload from
   * UploadStatus.UPLOAD_IN_PROGRESS to UploadStatus.UPLOAD_FAILED
   *
  ...
  public int failInProgressUploads(final UploadResult uploadResult) {
    ...
    ContentValues cv = new ContentValues();
    cv.put(ProviderTableMeta.UPLOADS_STATUS,
           UploadStatus.UPLOAD_FAILED.getValue());
    cv.put(ProviderTableMeta.UPLOADS_LAST_RESULT,
           uploadResult != null ? uploadResult.getValue()
                                : UploadResult.UNKNOWN.getValue());
    cv.put(ProviderTableMeta.UPLOADS_UPLOAD_END_TIMESTAMP,
           Calendar.getInstance().getTimeInMillis());

    int result =
        getDB().update(ProviderTableMeta.CONTENT_URI_UPLOADS, cv,
                       ProviderTableMeta.UPLOADS_STATUS + "=?",
                       new String[] {String.valueOf(
                           UploadStatus.UPLOAD_IN_PROGRESS.getValue())});
