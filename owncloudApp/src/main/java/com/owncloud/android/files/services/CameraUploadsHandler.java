...
/**
 * Schedule the periodic job responsible for camera uploads and initialize the
 * required information, as long as it matches the configuration for camera
 * uploads
 */
...
private CameraUploadsConfiguration
        mCameraUploadsConfig; // Camera uploads configuration, set by the user

public CameraUploadsHandler(
	final CameraUploadsConfiguration cameraUploadsConfiguration) {
...
public void scheduleCameraUploadsSyncJob(final Context context) {
	...
	CameraUploadsSyncStorageManager cameraUploadsSyncStorageManager =
		new CameraUploadsSyncStorageManager(context.getContentResolver());
	...
	if (ocCameraUploadSync == null ||
		...
		initializeCameraUploadSync(cameraUploadsSyncStorageManager,
		                           ocCameraUploadSync);
	}

	ComponentName serviceComponent =
		new ComponentName(context, CameraUploadsSyncJobService.class);
	...
	extras.putInt(Extras.EXTRA_CAMERA_UPLOADS_SYNC_JOB_ID,
	              JOB_ID_CAMERA_UPLOAD);

	extras.putString(Extras.EXTRA_ACCOUNT_NAME,
	                 mCameraUploadsConfig.getUploadAccountName());

	if (mCameraUploadsConfig.isEnabledForPictures()) {
		extras.putString(Extras.EXTRA_CAMERA_UPLOADS_PICTURES_PATH,
		                 mCameraUploadsConfig.getUploadPathForPictures());
	}

	if (mCameraUploadsConfig.isEnabledForVideos()) {
		extras.putString(Extras.EXTRA_CAMERA_UPLOADS_VIDEOS_PATH,
		                 mCameraUploadsConfig.getUploadPathForVideos());
	}

	extras.putString(Extras.EXTRA_CAMERA_UPLOADS_SOURCE_PATH,
	                 mCameraUploadsConfig.getSourcePath());

	extras.putInt(Extras.EXTRA_CAMERA_UPLOADS_BEHAVIOR_AFTER_UPLOAD,
	              mCameraUploadsConfig.getBehaviourAfterUpload());
...
/**
 * Initialize the timestamps for upload pictures/videos. These timestamps
 * define the start of the period in which to check the pictures/videos saved,
 * discarding those created before enabling Camera Uploads feature
 */
private void initializeCameraUploadSync(
	final CameraUploadsSyncStorageManager cameraUploadsSyncStorageManager,
	final OCCameraUploadSync ocCameraUploadSync) {

	// Set synchronization timestamps not needed
	if (!mCameraUploadsConfig.isEnabledForPictures() &&
	    !mCameraUploadsConfig.isEnabledForVideos()) {
		return;
	}
	...
	if (ocCameraUploadSync ==
	    null) { // No synchronization timestamp for pictures/videos yet

		long firstPicturesTimeStamp =
			mCameraUploadsConfig.isEnabledForPictures() ? timeStamp : 0;
		long firstVideosTimeStamp =
			mCameraUploadsConfig.isEnabledForVideos() ? timeStamp : 0;

		// Initialize synchronization timestamp for pictures or videos in database
		OCCameraUploadSync firstOcCameraUploadSync =
			new OCCameraUploadSync(firstPicturesTimeStamp, firstVideosTimeStamp);

		Timber.d("Storing synchronization timestamp in database");

		cameraUploadsSyncStorageManager.storeCameraUploadSync(
			firstOcCameraUploadSync);
		...
		if (ocCameraUploadSync.getVideosLastSync() == 0 &&
		    mCameraUploadsConfig.isEnabledForVideos()) {
...
/**
 * Update timestamp (in milliseconds) from which to start checking pictures to
 * upload
 *
 * @param lastSyncTimestamp
 */
public void updatePicturesLastSync(final Context context,
...
/**
 * Update timestamp (in milliseconds) from which to start checking videos to
 * upload
 *
 * @param lastSyncTimestamp
 */
public void updateVideosLastSync(final Context context,
                                 final long lastSyncTimestamp) {
	// DB connection
	CameraUploadsSyncStorageManager cameraUploadsSyncStorageManager =
		new CameraUploadsSyncStorageManager(context.getContentResolver());
...
public void setCameraUploadsConfig(
	final CameraUploadsConfiguration mCameraUploadsConfig) {
