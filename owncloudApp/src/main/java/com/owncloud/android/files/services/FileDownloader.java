...
private IndexedForest<DownloadFileOperation> mPendingDownloads =
	new IndexedForest<>();
...
public void onCreate() {
	...
	if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
		...
		mNotificationChannel = new NotificationChannel(
			DOWNLOAD_NOTIFICATION_CHANNEL_ID, name, importance);
	...
	HandlerThread thread = new HandlerThread(
		"FileDownloaderThread", Process.THREAD_PRIORITY_BACKGROUND);
	thread.start();
...
public int onStartCommand(final Intent intent, final int flags,
	...
	if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O &&
	    (isAvailableOfflineFile || retryDownload)) {
		/*
		 * We have to call this within five seconds after the service is created
		 * with startForegroundService when:
	...
	if (!intent.hasExtra(KEY_ACCOUNT) || !intent.hasExtra(KEY_FILE)) {
		...
		try {
			...
			newDownload.addDatatransferProgressListener(
				(FileDownloaderBinder)mBinder);
			...
			} // else, file already in the queue of downloads; don't repeat the
			  // request
...
/**
 * Provides a binder object that clients can use to perform operations on the
 * queue of downloads, excepting the addition of new files. <p/> Implemented
 * to perform cancellation, pause and resume of existing downloads.
 */
@Override
...
public void onAccountsUpdated(final Account[] accounts) {
	...
	if (mCurrentDownload != null &&
	    !AccountUtils.exists(mCurrentDownload.getAccount().name,
 ...
 * downloads. <p/> It provides by itself the available operations.
 */
public class FileDownloaderBinder
 ...
 * instance.
 */
private Map<Long, WeakReference<OnDatatransferProgressListener> >
...
public void cancel(final Account account) {
	...
	if (mCurrentDownload != null) {
		Timber.d("Current Download Account= %s",
		         mCurrentDownload.getAccount().name);
 ...
 *
 * If 'file' is a directory, returns 'true' if any of its descendant files
...
/**
 * Adds a listener interested in the progress of the download for a concrete
 * file.
 *
 ...
 * requested. Created with the Looper of a new thread, started in {@link
 * FileUploader#onCreate()}.
 */
...
private void downloadFile(final String downloadKey) {
	...
	if (mCurrentDownload != null) {
		...
		if (!AccountUtils.exists(mCurrentDownload.getAccount().name, this)) {
			Timber.w("Account " + mCurrentDownload.getAccount().name +
		...
		try {
			...
			if (mCurrentAccount == null ||
				...
				mStorageManager = new FileDataStorageManager(this, mCurrentAccount,
				                                             getContentResolver());
			...
			Pair<DownloadFileOperation, String> removeResult =
				mPendingDownloads.removePayload(mCurrentAccount.name,
				                                mCurrentDownload.getRemotePath());

			if (!downloadResult.isSuccess() &&
			    downloadResult.getException() != null) {
				...
				if (requester.shouldScheduleRetry(this,
					...
					int jobId = mPendingDownloads
					            .buildKey(mCurrentAccount.name,
					                      mCurrentDownload.getRemotePath())
					            .hashCode();
					requester.scheduleDownload(this, jobId, mCurrentAccount.name,
					                           mCurrentDownload.getRemotePath());
					downloadResult =
				...
				Timber.v("Success OR fail without exception for %1s in %2s",
				         mCurrentDownload.getRemotePath(), mCurrentAccount.name);
			}
...
private void saveDownloadedFile() {
	OCFile file =
		mStorageManager.getFileById(mCurrentDownload.getFile().getFileId());
...
private void notifyDownloadStart(final DownloadFileOperation download) {
	...
	.setContentText(String.format(
				getString(R.string.downloader_download_in_progress_content), 0,
				new File(download.getSavePath()).getName()))
	.setWhen(System.currentTimeMillis());

	/// includes a pending intent in the notification showing the details view
	/// of the file
...
onTransferProgress(final long progressRate, final long totalTransferredSoFar,
	...
	if (percent != mLastPercent) {
		...
		String text = String.format(
			getString(R.string.downloader_download_in_progress_content), percent,
			fileName);
 ...
 */
private void
notifyDownloadResult(final DownloadFileOperation download,
	...
	if (!downloadResult.isCancelled()) {
		...
		boolean needsToUpdateCredentials =
			(ResultCode.UNAUTHORIZED.equals(downloadResult.getCode()));
		...
		mNotificationBuilder.setContentText(
			ErrorMessageAdapter.Companion.getResultMessage(
				downloadResult, download, getResources()));
		...
		if (downloadResult.isSuccess()) {
			...
			NotificationUtils.cancelWithDelay(
				mNotificationManager, R.string.downloader_download_succeeded_ticker,
				2000);
		}
	}
}
 ...
 * @param download               Finished download operation
 * @param downloadResult         Result of the download operation
 * @param unlinkedFromRemotePath Path in the downloads tree where the download
 *     was unlinked from
 */
private void
sendBroadcastDownloadFinished(final DownloadFileOperation download,
                              final RemoteOperationResult downloadResult,
 ...
 * @param linkedToRemotePath Path in the downloads tree where the download was
 *     linked to
 */
