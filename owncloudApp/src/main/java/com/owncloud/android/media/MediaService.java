...
public class MediaService extends Service
  ...
  private static final String MY_PACKAGE =
      MediaService.class.getPackage() != null
          ? MediaService.class.getPackage().getName()
          : "com.owncloud.android.media";
  ...
  public static String EXTRA_START_POSITION =
      MY_PACKAGE + ".extra.START_POSITION";
  public static final String EXTRA_PLAY_ON_LOAD =
      MY_PACKAGE + ".extra.PLAY_ON_LOAD";

  /**
   * Error code for specific messages - see regular error codes at {@link
   * MediaPlayer}
   */
  ...
  enum State { STOPPED, PREPARING, PLAYING, PAUSED }

  /** Current state */
  private State mState = State.STOPPED;

  /** Possible focus values */
  enum AudioFocus { NO_FOCUS, NO_FOCUS_CAN_DUCK, FOCUS }
  ...
  /**
   * Notification builder to create notifications, new reuse way since Android
   * 6
   */
  private NotificationCompat.Builder mNotificationBuilder;

  /**
   * Helper method to get an error message suitable to show to users for errors
   * occurred in media playback,
   *
   ...
   */
  public static String getMessageForMediaError(final Context context,
                                               final int what,
                                               final int extra) {
    int messageId;
    ...
    if (what == OC_MEDIA_ERROR) {
      ...
      /*  Added in API level 17
          Bitstream is not conforming to the related coding standard or file
         spec. Constant Value: -1007 (0xfffffc11)
       */
      ...
      /*  Added in API level 17
         ...
         seconds. Constant Value: -110 (0xffffff92)
      */
      ...
      /*  Added in API level 3
         ...
         the file. Constant Value: 200 (0x000000c8)
      */
  ...
  public void onCreate() {
    ...
    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
      ...
      CharSequence name =
          getString(R.string.media_service_notification_channel_name);
      // The user-visible description of the channel.
      String description =
          getString(R.string.media_service_notification_channel_description);
      // Set importance low: show the notification everywhere but with no sound
      int importance = NotificationManager.IMPORTANCE_LOW;
      mNotificationChannel = new NotificationChannel(
          MEDIA_SERVICE_NOTIFICATION_CHANNEL_ID, name, importance);
    ...
    mAccountManager.addOnAccountsUpdatedListener(
        new OnAccountsUpdateListener() {
          ...
          public void onAccountsUpdated(final Account[] accounts) {
            ...
            if (mAccount != null &&
                !AccountUtils.exists(mAccount.name, MediaService.this)) {
  ...
  private void processPlayFileRequest(final Intent intent) {
    if (mState != State.PREPARING) {
      ...
      mPlayOnPrepared =
          intent.getExtras().getBoolean(EXTRA_PLAY_ON_LOAD, false);
  ...
  protected void processPlayRequest() {
    ...
    if (mState == State.STOPPED) {
      ...
      setUpAsForeground(String.format(getString(R.string.media_state_playing),
                                      mFile.getFileName()));
   ...
   * media player if needed. reset the existing media player if one already
   * exists.
   */
  protected void createMediaPlayerIfNeeded() {
    if (mPlayer == null) {
      ...
      // the media player will notify the service when it's ready preparing, and
   ...
   *
   * @param   force       When 'true', the playback is stopped no matter the
   *     value of mState
   */
  ...
  protected void configAndStartMediaPlayer() {
    ...
    if (mAudioFocus == AudioFocus.NO_FOCUS) {
      if (mPlayer.isPlaying()) {
        mPlayer.pause(); // have to be polite; but mState is not changed, to
                         // resume when focus is
                         // received again
      }
  ...
  protected void playMedia() {
    ...
    try {
      if (mFile == null) {
        Toast
            .makeText(this, R.string.media_err_nothing_to_play,
                      Toast.LENGTH_LONG)
            .show();
        processStopRequest(true);
        return;
      ...
      if (url == null || url.length() <= 0) {
          url = AccountUtils.constructFullURLForAccount(this, mAccount) +
      mFile.getRemotePath();
      }
      ...
      */
      ...
      setUpAsForeground(String.format(getString(R.string.media_state_loading),
                                      mFile.getFileName()));
      ...
      Timber.e(e, "SecurityException playing " + mAccount.name +
                      mFile.getRemotePath());
      Toast
          .makeText(this,
                    String.format(getString(R.string.media_err_security_ex),
                                  mFile.getFileName()),
                    Toast.LENGTH_LONG)
          .show();
      processStopRequest(true);
      ...
      Timber.e(e,
               "IOException playing " + mAccount.name + mFile.getRemotePath());
      Toast
          .makeText(this,
                    String.format(getString(R.string.media_err_io_ex),
                                  mFile.getFileName()),
                    Toast.LENGTH_LONG)
          .show();
      processStopRequest(true);
      ...
      Timber.e(e, "IllegalStateException " + mAccount.name +
                      mFile.getRemotePath());
      Toast
          .makeText(this,
                    String.format(getString(R.string.media_err_unexpected),
                                  mFile.getFileName()),
                    Toast.LENGTH_LONG)
          .show();
      processStopRequest(true);
      ...
      Timber.e(e, "IllegalArgumentException " + mAccount.name +
                      mFile.getRemotePath());
      Toast
          .makeText(this,
                    String.format(getString(R.string.media_err_unexpected),
  ...
  public void onCompletion(final MediaPlayer player) {
    Toast
        .makeText(this,
                  String.format(getString(R.string.media_event_done),
                                mFile.getFileName()),
                  Toast.LENGTH_LONG)
        .show();
  ...
  public void onPrepared(final MediaPlayer player) {
    mState = State.PLAYING;
    updateNotification(String.format(getString(R.string.media_state_playing),
                                     mFile.getFileName()));
    if (mMediaController != null) {
  ...
  private void updateNotification(final String content) {
    String ticker = String.format(getString(R.string.media_notif_ticker),
    ...
    mNotificationBuilder.setContentIntent(PendingIntent.getActivity(
        ...
        showDetailsIntent, PendingIntent.FLAG_UPDATE_CURRENT));
    ...
    mNotificationManager.notify(R.string.media_notif_ticker,
  ...
  public void onAudioFocusChange(final int focusChange) {
    if (focusChange > 0) {
      ...
      mAudioFocus =
          canDuck ? AudioFocus.NO_FOCUS_CAN_DUCK : AudioFocus.NO_FOCUS;
   ...
   * MediaPlayer managed by the MediaService.
   */
  @Override
   ...
   */
  protected MediaPlayer getPlayer() { return mPlayer; }
   ...
   */
  protected OCFile getCurrentFile() { return mFile; }
  ...
  protected MediaControlView getMediaController() { return mMediaController; }

  /**
   * Observer monitoring the media file currently played and stopping the
   * playback in case that it's deleted or moved away from its storage location.
   */
  private class MediaFileObserver extends FileObserver {

    public MediaFileObserver(final String path) {
      super((new File(path)).getParent(),
