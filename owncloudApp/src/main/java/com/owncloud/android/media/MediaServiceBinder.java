...
/**
 *  Binder allowing client components to perform operations on on the
 * MediaPlayer managed by a MediaService instance.
 *
 *  Provides the operations of {@link MediaController.MediaPlayerControl}, and
 * an extra method to check if an {@link OCFile} instance is handled by the
 * MediaService.
 */
public class MediaServiceBinder
	extends Binder implements MediaController.MediaPlayerControl {
 ...
 * Public constructor
 *
 * @param service       A {@link MediaService} instance to access with the
 *     binder
 */
 ...
 * the expected client of this method is a {@link MediaController} , and we do
 * not want that the 'play' button is shown when the file is being prepared by
 * the MediaService.
 */
@Override
public boolean isPlaying() {
	...
	return (currentState == State.PLAYING ||
	        (currentState == State.PREPARING && mService.mPlayOnPrepared));
}
...
public void seekTo(final int pos) {
	...
	MediaPlayer currentPlayer = mService.getPlayer();
	MediaService.State currentState = mService.getState();
	if (currentPlayer != null && currentState != State.PREPARING &&
...
public void start() {
	...
	mService.processPlayRequest(); // this will finish the service if there is
	                               // no file preloaded to play
}

public void start(final Account account, final OCFile file,
                  final boolean playImmediately, final int position) {
...
public void
unregisterMediaController(final MediaControlView mediaController) {
	if (mediaController != null &&
	    mediaController == mService.getMediaController()) {
...
public boolean isInPlaybackState() {
	MediaService.State currentState = mService.getState();
	return (currentState == MediaService.State.PLAYING ||
	        currentState == MediaService.State.PAUSED);
}
