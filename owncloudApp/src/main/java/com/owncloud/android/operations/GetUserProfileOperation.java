...
public class GetUserProfileOperation extends SyncOperation {
   ...
   */
  GetUserProfileOperation(final String remotePath) { mRemotePath = remotePath; }
   ...
   *              {@link String} with the display name retrieved from the
   * server. Call {@link RemoteOperationResult#getData()}.get(0) to get it.
   */
  @Override
  protected RemoteOperationResult run(final OwnCloudClient client) {
    ...
    try {
      ...
      if (useCaseResult.getDataOrNull() != null) {
        ...
        Timber.d("User info recovered from UseCaseHelper:GetUserInfo -> %s",
                 userInfo.toString());
        Account storedAccount = getStorageManager().getAccount();
        accountManager.setUserData(
            storedAccount,
            AccountUtils.Constants
                .KEY_DISPLAY_NAME, // keep also there, for the moment
            userInfo.getDisplayName());
        accountManager.setUserData(storedAccount, AccountUtils.Constants.KEY_ID,
                                   userInfo.getId());

        // map user info into UserProfile instance
        userProfile =
            new UserProfile(storedAccount.name, userInfo.getId(),
                            userInfo.getDisplayName(), userInfo.getEmail());

        /// get quota
        GetRemoteUserQuotaOperation getRemoteUserQuotaOperation =
            new GetRemoteUserQuotaOperation(mRemotePath);
        ...
        if (quotaOperationResult.isSuccess()) {
          ...
          UserProfile.UserQuota userQuota = new UserProfile.UserQuota(
              remoteQuota.getFree(), remoteQuota.getRelative(),
              remoteQuota.getTotal(), remoteQuota.getUsed());
          ...
          GetRemoteUserAvatarOperation getAvatarOperation =
              new GetRemoteUserAvatarOperation(dimension);

          RemoteOperationResult<GetRemoteUserAvatarOperation.ResultData>
              avatarOperationResult = getAvatarOperation.execute(client);

          if (avatarOperationResult.isSuccess()) {
            GetRemoteUserAvatarOperation.ResultData avatar =
                avatarOperationResult.getData();

            byte[] avatarData = avatar.getAvatarData();
            String avatarKey = ThumbnailsCacheManager.addAvatarToCache(
                storedAccount.name, avatarData, dimension);
          ...
          } else if (avatarOperationResult.getCode().equals(
          ...
          } // others are ignored, including 304 (not modified), so the avatar
            // is only stored
