...
/**
 * Operation performing a REFRESH on a folder, conceived to be triggered by an
 * action started FROM THE USER INTERFACE.
 *
 * Fetches the LIST and properties of the files contained in the given folder
 * (including the properties of the folder itself), and updates the local
 * database with them.
 *
 * Synchronizes the CONTENTS of any file or folder set locally as AVAILABLE
 * OFFLINE.
 *
 * If the folder is ROOT, it also retrieves the VERSION of the server, and the
 * USER PROFILE info.
 *
...
public static final String EVENT_SINGLE_FOLDER_CONTENTS_SYNCED =
	RefreshFolderOperation.class.getName() +
 ...
 */
private boolean mIgnoreETag;   // TODO - use it prefetching ETag of folder; two
                               // PROPFINDS, but better
 ...
 * @param ignoreETag       'True' means that the content of the remote folder
 *     should
 *                         be fetched and updated even though the 'eTag' did
 * not change.
 * @param account          ownCloud account where the folder is located.
 * @param context          Application context.
 */
public RefreshFolderOperation(final OCFile folder, final boolean ignoreETag,
...
run(final OwnCloudClient client) {
	...
	SynchronizeFolderOperation syncOp = new SynchronizeFolderOperation(
		mContext, mLocalFolder.getRemotePath(), mAccount,
		System.currentTimeMillis(), false, false, false);
	result = syncOp.execute(client, getStorageManager());

	sendLocalBroadcast(EVENT_SINGLE_FOLDER_CONTENTS_SYNCED,
	                   mLocalFolder.getRemotePath(), serverVersion, result);

	if (result.isSuccess()) {
		updateShareIconsInFiles(client); // share result is ignored
	}

	sendLocalBroadcast(EVENT_SINGLE_FOLDER_SHARES_SYNCED,
	                   mLocalFolder.getRemotePath(), serverVersion, result);
...
private void syncUserProfile() {
	GetUserProfileOperation update =
		new GetUserProfileOperation(mLocalFolder.getRemotePath());
	RemoteOperationResult result =
		update.execute(getStorageManager(), mContext);
	if (!result.isSuccess()) {
...
/**
 * Normally user profile and owncloud version get synchronized if you sync the
 * root directory. With this you can override this behaviour and disable it,
 * which is useful for the DocumentsProvider
 *
 * @param syncVersionAndProfileEnabled disables/enables sync Version/Profile
 *     when syncing root DIR
 */
public void
syncVersionAndProfileEnabled(final boolean syncVersionAndProfileEnabled) {
...
private OwnCloudVersion syncCapabilitiesAndGetServerVersion() {
	...
	RemoteOperationResult<RemoteCapability> result =
		getCapabilities.execute(getStorageManager(), mContext);
	if (result.isSuccess()) {
...
private void updateShareIconsInFiles(final OwnCloudClient client) {
	...
	GetRemoteSharesForFileOperation operation =
		new GetRemoteSharesForFileOperation(mLocalFolder.getRemotePath(), true,
		                                    true);
	...
	if (result.isSuccess()) {
		...
		for (RemoteShare remoteShare : result.getData().getShares()) {
			...
			if (file != null) {
				ShareType shareType = ShareType.Companion.fromValue(
					remoteShare.getShareType().getValue());
...
private void sendLocalBroadcast(final String event,
                                ...
                                final OwnCloudVersion serverVersion,
