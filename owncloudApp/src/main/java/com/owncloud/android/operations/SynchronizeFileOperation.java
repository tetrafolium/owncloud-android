...
/**
 * Operation synchronizing the properties and contents of an OC file between
 ...
 *
 * Uses remotePath to retrieve all the data both in local cache and in the
 * remote OC server when the operation is executed, instead of reusing {@link
 * OCFile} instances.
 *
 ...
 */
public SynchronizeFileOperation(final String remotePath,
...
/**
 * Constructor allowing to reuse {@link OCFile} instances just queried from
 * local cache or from remote OC server.
 *
 ...
 *
 * At least one of localFile or serverFile MUST NOT BE NULL. If you don't have
 * none of them, use the other constructor.
 *
 * @param localFile        Data of file (just) retrieved from local
 *     cache/database.
 * @param serverFile       Data of file (just) retrieved from a remote server.
 *     If null,
 *                         will be retrieved from network by the operation
 * when executed.
 * @param account          ownCloud account holding the file.
 * @param pushOnly         When 'true', if 'severFile' is NULL, will not fetch
 *     remote properties before
 *                         trying to upload local changes; upload operation
 * will take care of not overwriting remote content if there are unnoticed
 * changes on the server.
 * @param context          Android context; needed to start transfers.
 * @param requestedFromAvOfflineJobService When 'true' will perform some
 *     specific operations
 */
public SynchronizeFileOperation(
	final OCFile localFile, final OCFile serverFile, final Account account,
	final boolean pushOnly, final Context context,
	...
	if (mLocalFile != null) {
		...
		if (mServerFile != null &&
		    !mServerFile.getRemotePath().equals(mRemotePath)) {
			throw new IllegalArgumentException(
				      "serverFile and localFile do not correspond"
				      + " to the same OC file");
		}
		...
		throw new IllegalArgumentException(
			      "Both serverFile and localFile are NULL");
	}
	mAccount = account;
	mPushOnly = pushOnly;
	mContext = context;
...
protected RemoteOperationResult run(final OwnCloudClient client) {
	...
	if (!mLocalFile.isDown()) {
		...
		if (mServerFile == null && !mPushOnly) {
			...
			if (result.isSuccess()) {
				mServerFile =
					FileStorageUtils.createOCFileFromRemoteFile(result.getData());
		...
		if (mPushOnly ||
		    mServerFile !=
		    null) { // at this point, conditions should be exclusive

			/// decide if file changed in the server
			boolean serverChanged;
			if (mPushOnly) {
				serverChanged = false;
			} else if (mLocalFile.getEtag() == null ||
			           mLocalFile.getEtag().length() == 0) {
			...
			boolean localChanged = (mLocalFile.getLocalModificationTimestamp() >
			...
			// safe blanket: sync'ing a not in-conflict file will clean wrong
			// conflict markers in ancestors
	...
	if (MainApp.Companion.isDeveloper()) {
		Timber.i("Synchronizing " + mAccount.name + ", file " +
...
private void requestForUpload(final OCFile file) {
	...
	requester.uploadUpdate(mContext, mAccount, file,
	                       FileUploader.LOCAL_BEHAVIOUR_MOVE, true,
...
private void requestForDownload(final OCFile file) {
	...
	if (mRequestedFromAvOfflineJobService &&
	    Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
		intent.putExtra(FileDownloader.KEY_IS_AVAILABLE_OFFLINE_FILE, true);
		Timber.d(
			"Download file from foreground/background, startForeground() will be called soon");
