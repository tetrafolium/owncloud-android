 ...
 * including their properties, and updates the local database with them. <p>
 * Does NOT enter in the child folders to synchronize their contents also, BUT
 * requests for a new operation instance doing so.
 */
public class SynchronizeFolderOperation
    extends SyncOperation<ArrayList<RemoteFile>> {
  ...
  /**
   * 'True' means that the contents of all the files in the folder will be
   * synchronized; otherwise, only contents of available offline files will be
   * synchronized.
   */
   ...
   * @param account                   ownCloud account where the folder is
   *     located.
   * @param currentSyncTime           Time stamp for the synchronization process
   *     in progress.
   * @param pushOnly                  When 'true', will assume that folder did
   *     not change in the server and
   *                                  will focus only in push any local change
   * to the server (carefully).
   * @param syncFullAccount           'True' means that this operation is part
   *     of a full account
   *                                  synchronization.
   * @param syncContentOfRegularFiles When 'true', the contents of all the files
   *     in the folder will
   *                                  be synchronized; otherwise, only contents
   * of available offline files will be synchronized.
   */
  public SynchronizeFolderOperation(
      final Context context, final String remotePath, final Account account,
      final long currentSyncTime, final boolean pushOnly,
      final boolean syncFullAccount, final boolean syncContentOfRegularFiles) {
  ...
  public int getConflictsFound() { return mConflictsFound; }

  public int getFailsInFileSyncsFound() { return mFailsInFileSyncsFound; }
  ...
  /**
   * Returns the list of subfolders after the refresh, in a {@link Pair} with a
   * boolean indicating if was detected as changed in the server or not.
   *
   * @return List of pairs of subfolders and boolean flags set to 'true' if
   *     there are pending
   ...
   */
  @Override
  protected RemoteOperationResult<ArrayList<RemoteFile>>
  run(final OwnCloudClient client) {
    ...
    try {
      ...
      if (mPushOnly) {
        // assuming there is no update in the server side, still need to handle
        // local changes
        ...
        if (fetchFolderResult.isSuccess()) {
          ...
          if (fetchFolderResult.isException()) {
            Timber.e(fetchFolderResult.getException(),
                     "Checked " + mAccount.name + mRemotePath + " : " +
                         fetchFolderResult.getLogMessage());
          } else {
            Timber.e("Checked " + mAccount.name + mRemotePath + " : " +
                     fetchFolderResult.getLogMessage());
          }
        }
      }
   ...
   * @return Result of the fetch, including list of remote files in the sync'ed
   *     folder.
   * @throws OperationCancelledException
   */
  @NonNull
  private RemoteOperationResult<ArrayList<RemoteFile>>
  fetchRemoteFolder(final OwnCloudClient client)
      throws OperationCancelledException {
    Timber.d("Fetching list of files in  " + mAccount.name + mRemotePath +
             ", if changed");
    ...
    ReadRemoteFolderOperation readFolderOperation =
        new ReadRemoteFolderOperation(mRemotePath);
    return readFolderOperation.execute(client);
  }

  /**
   * Compares stored ETag of folder being synchronized to determine if there
   * were changes in the server from the last sync.
   *
   ...
   * target folder with the current data in the local database. <p> Grants that
   * mFoldersToVisit is updated with fresh data after execution.
   *
   * @param remoteFolderAndFiles Remote folder and children files in folder
   */
  private void
  mergeRemoteFolder(final ArrayList<RemoteFile> remoteFolderAndFiles)
      throws OperationCancelledException {
    ...
    OCFile updatedFolder =
        FileStorageUtils.createOCFileFromRemoteFile(remoteFolderAndFiles.get(
            0)); // NOTE: updates ETag with remote value; that's INTENDED
    updatedFolder.copyLocalPropertiesFrom(mLocalFolder);

    Timber.d("Remote folder " + mLocalFolder.getRemotePath() +
    ...
    for (OCFile file : localFiles) {
      String key = file.getRemoteId() != null ? file.getRemoteId()
                                              : file.getRemotePath();
      if (key != null) {
    ...
    for (int i = 1; i < remoteFolderAndFiles.size(); i++) {
      ...
      remoteFile = FileStorageUtils.createOCFileFromRemoteFile(
          remoteFolderAndFiles.get(i));
      ...
      updatedLocalFile = FileStorageUtils.createOCFileFromRemoteFile(
          remoteFolderAndFiles.get(i));
      ...
      if (localFile != null) {
        ...
        if (!updatedLocalFile.isFolder() && remoteFile.isImage() &&
    ...
    storageManager.saveFolder(updatedFolder, updatedFiles,
                              localFilesMap.values());
  }
  ...
  private void preparePushOfLocalChanges() {
    ...
    for (OCFile child : children) {
      addToSyncContents(child, null);
    }
  }

  /**
   * Generates the appropriate operations to later sync the contents of
   * localFile with the server. <p> Stores the operations in
   * mFoldersToSyncContents and mFilesToSyncContents.
   *
   * @param localFile  Local information about the file which contents might be
   *     sync'ed.
   * @param remoteFile Server information of the file.
   * @return 'True' when the received file was not changed in the server side
   *     from the
   * last synchronization.
   */
  private boolean addToSyncContents(final OCFile localFile,
                                    final OCFile remoteFile) {

    boolean shouldSyncContents =
        (mSyncContentOfRegularFiles || localFile.isAvailableOffline());
    ...
    if (localFile.isFolder()) {
      ...
      serverUnchanged = (remoteFile == null) ||
      ...
      if (mSyncFullAccount) {
        ...
        intent.putExtra(OperationsService.EXTRA_REMOTE_PATH,
                        localFile.getRemotePath());
        intent.putExtra(OperationsService.EXTRA_PUSH_ONLY, serverUnchanged);
        intent.putExtra(OperationsService.EXTRA_SYNC_REGULAR_FILES,
      ...
      if (shouldSyncContents && !isBlockedForAutomatedSync(localFile)) {
        ...
        SynchronizeFileOperation operation = new SynchronizeFileOperation(
            localFile, remoteFile, mAccount, serverUnchanged, mContext, false);
  ...
  /**
   * Performs a list of synchronization operations, determining if a download or
   * upload is needed or if exists conflict due to changes both in local and
   * remote contents of the each file. <p> If download or upload is needed,
   * request the operation to the corresponding service and goes on.
   */
      ...
      if (!contentsResult.isSuccess()) {
        if (contentsResult.getCode() == ResultCode.SYNC_CONFLICT) {
          ...
          if (contentsResult.getException() != null) {
            Timber.e(contentsResult.getException(),
                     "Error while synchronizing file : %s",
                     contentsResult.getLogMessage());
          } else {
            Timber.e("Error while synchronizing file : %s",
                     contentsResult.getLogMessage());
          }
        }
   ...
   * parameter.
   * <p>
   * This method helps to keep linked local copies of the files when the app is
   * uninstalled, and then reinstalled in the device. OR after the cache of the
   * app was deleted in system settings. <p> The method is assuming that all the
   * local changes in the file where synchronized in the past. This is
   * dangerous, but assuming the contrary could lead to massive unnecessary
   * synchronizations of downloaded file after deleting the app cache. <p> This
   * should be changed in the near future to avoid any chance of data loss, but
   * we need to add some options to limit hard automatic synchronizations to
   * wifi, unless the user wants otherwise.
   *
  ...
  /**
   * Checks the last upload of a file and determines if automated
   * synchronization needs to wait for user action or not.
   *
   * @param file ownCloud file to check.
   * @return 'True' if the received file should not be automatically sync'ed due
   *     to a previous
   * upload error that requires an user action.
   */
  private boolean isBlockedForAutomatedSync(final OCFile file) {
    UploadsStorageManager uploadsStorageManager =
        new UploadsStorageManager(mContext.getContentResolver());
    OCUpload failedUpload =
        uploadsStorageManager.getLastUploadFor(file, mAccount.name);
