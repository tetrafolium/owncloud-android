...
public class OperationsService extends Service {
  ...
  public static final String EXTRA_IS_LAST_FILE_TO_REMOVE =
      "EXTRA_IS_LAST_FILE_TO_REMOVE";
  ...
  public static final String ACTION_CHECK_CURRENT_CREDENTIALS =
      "CHECK_CURRENT_CREDENTIALS";

  public static final String ACTION_OPERATION_ADDED =
      OperationsService.class.getName() + ".OPERATION_ADDED";
  public static final String ACTION_OPERATION_FINISHED =
      OperationsService.class.getName() + ".OPERATION_FINISHED";
  ...
  private static class Target {
    ...
    public Target(final Account account, final Uri serverUrl,
                  final String cookie) {
      mAccount = account;
  ...
  public void onCreate() {
    ...
    thread = new HandlerThread("Syncfolder thread",
   ...
   *
   * New operations are added calling to startService(), resulting in a call to
  ...
  public void onDestroy() {
    ...
    // Saving cookies
    SingleSessionManager.getDefaultSingleton().saveAllClients(
        this, MainApp.Companion.getAccountType());
  ...
  /**
   * Provides a binder object that clients can use to perform actions on the
   * queue of operations, except the addition of new operations.
   */
  @Override
  ...
  /**
   * Binder to let client components to perform actions on the queue of
   * operations. <p/> It provides by itself the available operations.
   */
  public class OperationsServiceBinder extends Binder {
     ...
     */
    private final ConcurrentMap<OnRemoteOperationListener, Handler>
        mBoundListeners = new ConcurrentHashMap<>();
    ...
    void clearListeners() { mBoundListeners.clear(); }
     ...
     */
    public void addOperationListener(final OnRemoteOperationListener listener,
    ...
    /**
     * Removes a listener from the list of objects interested in the being
     * reported about the end of operations.
     *
     * @param listener Object to notify about progress of transfer.
     */
    public void
    removeOperationListener(final OnRemoteOperationListener listener) {
      synchronized (mBoundListeners) { mBoundListeners.remove(listener); }
    }

    /**
     * Creates and adds to the queue a new operation, as described by
     * operationIntent.
     *
     * Calls startService to make the operation is processed by the
     * ServiceHandler.
     *
     * @param operationIntent Intent describing a new operation to queue and
     *     execute.
    ...
    public long queueNewOperation(final Intent operationIntent) {
      ...
      if (itemToQueue != null) {
        ...
        Intent executeOperation =
            new Intent(OperationsService.this, OperationsService.class);
        executeOperation.putExtra(EXTRA_IS_LAST_FILE_TO_REMOVE,
                                  operationIntent.getBooleanExtra(
                                      EXTRA_IS_LAST_FILE_TO_REMOVE, false));
    ...
    public boolean
    dispatchResultIfFinished(final int operationId,
                             final OnRemoteOperationListener listener) {
      Pair<RemoteOperation, RemoteOperationResult> undispatched =
          mUndispatchedFinishedOperations.remove(operationId);
      if (undispatched != null) {
        listener.onRemoteOperationFinish(undispatched.first,
                                         undispatched.second);
        return true;
      } else {
    ...
    /**
     * Returns True when the file described by 'file' in the ownCloud account
     * 'account' is downloading or waiting to download.
     *
  ...
  /**
   * Operations worker. Performs the pending operations in the order they were
   * requested.
   *
   * Created with the Looper of a new thread, started in {@link
   * OperationsService#onCreate()}.
   */
  private static class ServiceHandler extends Handler {
    // don't make it a final class, and don't remove the static ; lint will warn
    // about a p ossible memory leak

    OperationsService mService;

    private final ConcurrentLinkedQueue<Pair<Target, RemoteOperation>>
        mPendingOperations = new ConcurrentLinkedQueue<>();
    ...
    private void nextOperation() {
      ...
      synchronized (mPendingOperations) { next = mPendingOperations.peek(); }
      ...
      if (next != null) {
        ...
        try {
          ...
          if (mLastTarget == null || !mLastTarget.equals(next.first)) {
            ...
            if (mLastTarget.mAccount != null) {
              ...
              OwnCloudVersion version =
                  com.owncloud.android.authentication.AccountUtils
                      .getServerVersion(mLastTarget.mAccount);
              mOwnCloudClient.setOwnCloudVersion(version);

              mStorageManager =
                  new FileDataStorageManager(mService, mLastTarget.mAccount,
                                             mService.getContentResolver());
            } else {
              OwnCloudCredentials credentials = null;
              ocAccount =
                  new OwnCloudAccount(mLastTarget.mServerUrl, credentials);
          ...
          if (currentOperation instanceof SyncOperation) {
            result = ((SyncOperation)currentOperation)
                         .execute(mOwnCloudClient, mStorageManager);
          } else {
          ...
          if (mLastTarget.mAccount == null) {
            ...
            Timber.e(e, "Error while trying to get authorization for %s",
                     mLastTarget.mAccount.name);
          }
          ...
          synchronized (mPendingOperations) { mPendingOperations.poll(); }
        }
   ...
   * target server.
   */
  private Pair<Target, RemoteOperation>
  newOperation(final Intent operationIntent) {
    ...
    try {
      if (!operationIntent.hasExtra(EXTRA_ACCOUNT) &&
        ...
        target = new Target(
            account, (serverUrl == null) ? null : Uri.parse(serverUrl), cookie);
        ...
        if (action != null) {
          ...
          case ACTION_REMOVE: {
            ...
            boolean onlyLocalCopy =
                operationIntent.getBooleanExtra(EXTRA_REMOVE_ONLY_LOCAL, false);
            operation = new RemoveFileOperation(
                remotePath, onlyLocalCopy,
                operationIntent.getBooleanExtra(EXTRA_IS_LAST_FILE_TO_REMOVE,
                                                false));
          ...
          case ACTION_CREATE_FOLDER: {
            ...
            boolean createFullPath =
                operationIntent.getBooleanExtra(EXTRA_CREATE_FULL_PATH, true);
          ...
          case ACTION_SYNC_FILE: {
            ...
            String remotePath =
                operationIntent.getStringExtra(EXTRA_REMOTE_PATH);
            operation = new SynchronizeFileOperation(remotePath, account,
          ...
          case ACTION_SYNC_FOLDER: {
            ...
            String remotePath =
                operationIntent.getStringExtra(EXTRA_REMOTE_PATH);
            boolean pushOnly =
                operationIntent.getBooleanExtra(EXTRA_PUSH_ONLY, false);
            boolean syncContentOfRegularFiles = operationIntent.getBooleanExtra(
                EXTRA_SYNC_REGULAR_FILES, false);
            operation = new SynchronizeFolderOperation(
                this, // TODO remove this dependency from construction time
                remotePath, account,
                System.currentTimeMillis(), // TODO remove this dependency from
                                            // construction time
          ...
          case ACTION_MOVE_FILE: {
            ...
            String newParentPath =
                operationIntent.getStringExtra(EXTRA_NEW_PARENT_PATH);
          ...
          case ACTION_CHECK_CURRENT_CREDENTIALS:
            // Check validity of currently stored credentials for a given
            // account
   ...
   */
  protected void
  dispatchResultToOperationListeners(final RemoteOperation operation,
                                     final RemoteOperationResult result) {
    int count = 0;
    for (OnRemoteOperationListener listener :
         mOperationsBinder.mBoundListeners.keySet()) {
