...
/**
 * Base class to handle setup of the drawer implementation including user
 * switching and avatar fetching and fallback generation.
 */
public abstract class DrawerActivity extends ToolbarActivity {

  private static final String KEY_IS_ACCOUNT_CHOOSER_ACTIVE =
      "IS_ACCOUNT_CHOOSER_ACTIVE";
  ...
  protected void setupDrawer() {
    ...
    if (mNavigationView != null) {
      ...
      if (mDrawerLogo != null &&
          getResources().getBoolean(R.bool.use_drawer_logo)) {
        mDrawerLogo.setImageResource(R.drawable.drawer_logo);
      }

      mAccountChooserToggle = (ImageView)findNavigationViewChildById(
          R.id.drawer_account_chooser_toogle);
      ...
      // Notch support
      mNavigationView.addOnAttachStateChangeListener(
            ...
            public void onViewAttachedToWindow(final View v) {
              if (android.os.Build.VERSION.SDK_INT >=
                ...
                DisplayCutout displayCutout =
                    v.getRootWindowInsets().getDisplayCutout();

                if (displayCutout != null) {
                  ConstraintLayout rlDrawerActiveUser =
                      (ConstraintLayout)findNavigationViewChildById(
                          R.id.drawer_active_user);
                  ...
                  if (orientation == Configuration.ORIENTATION_PORTRAIT) {
                    ...
                    rlDrawerActiveUser.getLayoutParams().height =
                        (int)getResources().getDimension(
                            R.dimen.nav_drawer_header_height) +
                        displayCutoutDP;
                  } else {
                    rlDrawerActiveUser.getLayoutParams().height =
                        (int)getResources().getDimension(
    ...
    mDrawerToggle = new ActionBarDrawerToggle(
      ...
      public void onDrawerClosed(final View view) {
        ...
        // standard behavior of drawer is to switch to the standard menu on
        // closing
  ...
  protected void setupDrawerContent(final NavigationView navigationView) {
    ...
    if (mIsAccountChooserActive) {
      ...
      navigationView.getMenu().setGroupVisible(R.id.drawer_menu_accounts,
                                               false);
    }
  }
  ...
  /**
   * Initializes the bottom navigation bar, its content and highlights the menu
   * item with the given id. This method needs to be called after the content
   * view has been set.
   *
  ...
  private void navBarNavigationTo(final int menuItemId,
                                  final boolean isCurrentOptionActive) {
    ...
    case R.id.nav_uploads:
      if (!isCurrentOptionActive) {
        Intent uploadListIntent =
            new Intent(getApplicationContext(), UploadListActivity.class);
  ...
  private void openFeedback() {
    ...
    String feedback = "Android v" + BuildConfig.VERSION_NAME + " - " +
                      getText(R.string.drawer_feedback);
  ...
  /**
   * sets the new/current account and restarts. In case the given account equals
   * the actual/current account the call will be ignored.
   *
   * @param accountName The account name to be set
   */
  private void accountClicked(final String accountName) {
    if (!AccountUtils.getCurrentOwnCloudAccount(getApplicationContext())
             .name.equals(accountName)) {
      AccountUtils.setCurrentOwnCloudAccount(getApplicationContext(),
                                             accountName);
   ...
   *                 {@link DrawerLayout#LOCK_MODE_LOCKED_CLOSED} or {@link
  ...
  private void repopulateAccountList(final Account[] accounts) {
    ...
    for (Account account : accounts) {
      if (!getAccount().name.equals(account.name)) {
        ...
        MenuItem accountMenuItem =
            mNavigationView.getMenu().add(R.id.drawer_menu_accounts, Menu.NONE,
                                          MENU_ORDER_ACCOUNT, account.name);
        ThumbnailsCacheManager.GetAvatarTask task =
            new ThumbnailsCacheManager.GetAvatarTask(
                accountMenuItem, account, mMenuAccountAvatarRadiusDimension,
                false);
        task.execute();
      }
    }
    ...
    mNavigationView.getMenu()
        .add(R.id.drawer_menu_accounts, R.id.drawer_menu_account_add,
             MENU_ORDER_ACCOUNT_FUNCTION,
             getResources().getString(R.string.prefs_add_account))
        .setIcon(R.drawable.ic_plus_grey);
    mNavigationView.getMenu()
        .add(R.id.drawer_menu_accounts, R.id.drawer_menu_account_manage,
             MENU_ORDER_ACCOUNT_FUNCTION,
             getResources().getString(R.string.drawer_manage_accounts))
        .setIcon(R.drawable.ic_group);

    // adding sets menu group back to visible, so safety check and setting
    // invisible
    showMenu();
  }
  ...
  private void updateQuota() {
    ...
    UserProfile.UserQuota userQuota =
        UserProfilesRepository.getUserProfilesRepository().getQuota(
            account.name);
    ...
    if (accountQuotaBar != null && accountQuotaText != null) {
      ...
      if (userQuota.getFree() <
          0) { // Pending, unknown or unlimited free storage

        accountQuotaBar.setVisibility(View.VISIBLE);

        accountQuotaBar.setProgress(0);

        accountQuotaText.setText(String.format(
        ...
        // Update progress bar rounding up to next int. Example: quota is 0.54
        // => 1
  ...
  /**
   * Sets the given account data in the drawer in case the drawer is available.
   * The account name is shortened beginning from the @-sign in the username.
   *
  ...
  protected void setAccountInDrawer(final Account account) {
    if (mDrawerLayout != null && account != null) {
      TextView username =
          (TextView)findNavigationViewChildById(R.id.drawer_username);
      TextView usernameFull =
          (TextView)findNavigationViewChildById(R.id.drawer_username_full);
      ...
      try {
        ...
        Timber.w(
            "Couldn't read display name of account; using account name instead");
      ...
      DisplayUtils.showAccountAvatar(
          ...
          mCurrentAccountAvatarRadiusDimension, false);
  ...
  /**
   * depending on the #mIsAccountChooserActive flag shows the account chooser or
   * the standard menu.
   */
    ...
    if (mNavigationView != null) {
      final int accountCount =
          AccountManager.get(this)
              .getAccountsByType(MainApp.Companion.getAccountType())
              .length;

      if (mIsAccountChooserActive) {
        mAccountChooserToggle.setImageResource(R.drawable.ic_up);
        mNavigationView.getMenu().setGroupVisible(R.id.drawer_menu_accounts,
                                                  true);
        mNavigationView.getMenu().setGroupVisible(R.id.drawer_menu_settings_etc,
                                                  false);
        if (mDrawerLogo != null &&
            accountCount > USER_ITEMS_ALLOWED_BEFORE_REMOVING_CLOUD) {
  ...
  /**
   * checks/highlights the provided menu item if the drawer has been initialized
   * and the menu item exists.
   *
  ...
  protected void setDrawerMenuItemChecked(final int menuItemId) {
    if (mNavigationView != null && mNavigationView.getMenu() != null &&
      ...
      Timber.w(
          "setDrawerMenuItemChecked has been called with invalid menu-item-ID");
    }
  }
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      mIsAccountChooserActive =
          savedInstanceState.getBoolean(KEY_IS_ACCOUNT_CHOOSER_ACTIVE, false);
      mCheckedMenuItem =
          savedInstanceState.getInt(KEY_CHECKED_MENU_ITEM, Menu.NONE);
    }

    mCurrentAccountAvatarRadiusDimension =
        getResources().getDimension(R.dimen.nav_drawer_header_avatar_radius);
    mOtherAccountAvatarRadiusDimension = getResources().getDimension(
        R.dimen.nav_drawer_header_avatar_other_accounts_radius);
  ...
  protected void onActivityResult(final int requestCode, final int resultCode,
    ...
    // update Account list and active account if Manage Account activity replies
    // with
    // - ACCOUNT_LIST_CHANGED = true
    // - RESULT_OK
    if (requestCode == ACTION_MANAGE_ACCOUNTS && resultCode == RESULT_OK &&
        data.getBooleanExtra(ManageAccountsActivity.KEY_ACCOUNT_LIST_CHANGED,
                             false)) {

      // current account has changed
      if (data.getBooleanExtra(
              ManageAccountsActivity.KEY_CURRENT_ACCOUNT_CHANGED, false)) {
  ...
  /**
   * Finds a view that was identified by the id attribute from the drawer
   * header.
   *
  ...
  @Override
  protected void
  onAccountCreationSuccessful(final AccountManagerFuture<Bundle> future) {
  ...
  /**
   * populates the avatar drawer array with the first three ownCloud {@link
   * Account}s while the first element is always the current account.
   */
  private void populateDrawerOwnCloudAccounts() {
    ...
    int j = 0;
    for (int i = 1; i <= 2 && i < accountsAll.length && j < accountsAll.length;
         j++) {
