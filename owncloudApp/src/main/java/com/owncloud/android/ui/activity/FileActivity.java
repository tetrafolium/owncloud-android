...
public class FileActivity extends DrawerActivity
  ...
  public static final int REQUEST_CODE__LAST_SHARED =
  ...
  /**
   * Messages handler associated to the main thread and the life cycle of the
   * activity
   */
  ...
  /**
   * Loads the ownCloud {@link Account} and main {@link OCFile} to be handled by
   * the instance of the {@link FileActivity}.
   *
   * Grants that a valid ownCloud {@link Account} is associated to the instance,
   * or that the user is requested to create a new one.
   */
  @Override
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      ...
      mFromNotification =
          savedInstanceState.getBoolean(FileActivity.EXTRA_FROM_NOTIFICATION);
      ...
      mFromNotification = getIntent().getBooleanExtra(
          FileActivity.EXTRA_FROM_NOTIFICATION, false);
    }

    AccountUtils.updateAccountVersion(
        this); // best place, before any access to AccountManager
    // or database

    setAccount(account, savedInstanceState != null);

    mOperationsServiceConnection = new OperationsServiceConnection();
    bindService(new Intent(this, OperationsService.class),
                mOperationsServiceConnection, Context.BIND_AUTO_CREATE);

    mDownloadServiceConnection = newTransferenceServiceConnection();
    if (mDownloadServiceConnection != null) {
      bindService(new Intent(this, FileDownloader.class),
                  mDownloadServiceConnection, Context.BIND_AUTO_CREATE);
    }
    mUploadServiceConnection = newTransferenceServiceConnection();
    if (mUploadServiceConnection != null) {
      bindService(new Intent(this, FileUploader.class),
                  mUploadServiceConnection, Context.BIND_AUTO_CREATE);
    }
  }
  ...
  protected void onSaveInstanceState(final Bundle outState) {
    ...
    outState.putBoolean(FileActivity.EXTRA_FROM_NOTIFICATION,
                        mFromNotification);
    outState.putLong(KEY_WAITING_FOR_OP_ID,
                     mFileOperationsHelper.getOpIdWaitingFor());
    if (getSupportActionBar() != null &&
        getSupportActionBar().getTitle() != null) {
      // Null check in case the actionbar is used in
      // ActionBar.NAVIGATION_MODE_LIST since it doesn't have a title then
      outState.putString(KEY_ACTION_BAR_TITLE,
   ...
   */
  public void setFile(final OCFile file) { mFile = file; }

  /**
   * @return Value of mFromNotification: True if the Activity is launched by a
   *     notification
   */
  public boolean fromNotification() { return mFromNotification; }
  ...
  public OnRemoteOperationListener getRemoteOperationListener() { return this; }
  ...
  public void onRemoteOperationFinish(final RemoteOperation operation,
    ...
    if (!result.isSuccess() &&
        (result.getCode() == ResultCode.UNAUTHORIZED ||
         (result.isException() && result.getException() instanceof
      ...
      if (result.getCode() == ResultCode.UNAUTHORIZED) {
        showSnackMessage(ErrorMessageAdapter.Companion.getResultMessage(
            result, operation, getResources()));
               ...
               ResultCode.SSL_RECOVERABLE_PEER_UNVERIFIED.equals(
                   result.getCode())) {

      showUntrustedCertDialog(result);

    } else if (operation == null || operation instanceof
  ...
  protected void showRequestAccountChangeNotice(final String errorMessage,
    ...
    if (mustChange) {
      ...
      Snackbar
          .make(findViewById(android.R.id.content), errorMessage,
                Snackbar.LENGTH_INDEFINITE)
          .setAction(R.string.auth_failure_snackbar_action,
                     v
                     -> startActivity(new Intent(FileActivity.this,
                                                 ManageAccountsActivity.class)))
          .show();
  ...
  protected void showRequestRegainAccess() {
    Snackbar
        .make(findViewById(android.R.id.content), R.string.auth_oauth_failure,
              Snackbar.LENGTH_INDEFINITE)
        .setAction(R.string.auth_oauth_failure_snackbar_action,
                   v -> requestCredentialsUpdate())
        .show();
  }

  /**
   * Invalidates the credentials stored for the current OC account and requests
   * new credentials to the user, navigating to {@link LoginActivity}
   *
   * Equivalent to call requestCredentialsUpdate(null);
   */
  protected void requestCredentialsUpdate() { requestCredentialsUpdate(null); }

  /**
   * Invalidates the credentials stored for the given OC account and requests
   * new credentials to the user, navigating to {@link LoginActivity}
   *
   * @param account Stored OC account to request credentials update for. If
   *     null, current account will
   *                be used.
   */
  protected void requestCredentialsUpdate(final Account account) {
    ...
    startActivityForResult(updateAccountCredentials,
                           REQUEST_CODE__UPDATE_CREDENTIALS);
  }
  ...
  public void showUntrustedCertDialog(final RemoteOperationResult result) {
    ...
    SslUntrustedCertDialog dialog =
        (SslUntrustedCertDialog)fm.findFragmentByTag(DIALOG_UNTRUSTED_CERT);
    if (dialog == null) {
  ...
  private void doOnResumeAndBound() {
    ...
    if (waitingForOpId <= Integer.MAX_VALUE) {
      boolean wait = mOperationsServiceBinder.dispatchResultIfFinished(
          (int)waitingForOpId, this);
      if (!wait) {
        dismissLoadingDialog();
      }
    }
  ...
  private class OperationsServiceConnection implements ServiceConnection {
    ...
    public void onServiceConnected(final ComponentName component,
      ...
      if (component.equals(
              new ComponentName(FileActivity.this, OperationsService.class))) {
  ...
  public void navigateToOption(final FileListOption fileListOption) {
    ...
    case SHARED_BY_LINK:
      ...
      intent.putExtra(EXTRA_FILE_LIST_OPTION,
                      (Parcelable)FileListOption.SHARED_BY_LINK);
    ...
    case AV_OFFLINE:
      ...
      intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
      intent.putExtra(EXTRA_FILE_LIST_OPTION,
                      (Parcelable)FileListOption.AV_OFFLINE);
      startActivity(intent);
      break;
    }
  }
  ...
  public void onFailedSavingCertificate() {
    ConfirmationDialogFragment dialog = ConfirmationDialogFragment.newInstance(
        R.string.ssl_validator_not_saved, new String[] {}, 0,
