...
protected void onCreate(final Bundle savedInstanceState) {
	...
	Account[] accountList = AccountManager.get(this).getAccountsByType(
...
protected void onStart() {
	super.onStart();
	mAccountListAdapter =
		new AccountListAdapter(this, getAccountListItems(), mTintedCheck);
...
public void onBackPressed() {
	...
	resultIntent.putExtra(KEY_CURRENT_ACCOUNT_CHANGED,
...
/**
 * checks the set of actual accounts against the set of original accounts when
 * the activity has been started.
 *
 * @return <code>true</code> if aacount list has changed, <code>false</code>
 *     if not
 */
...
/**
 * checks actual current account against current accounts when the activity
 * has been started.
 *
 * @return <code>true</code> if aacount list has changed, <code>false</code>
 *     if not
...
private boolean hasCurrentAccountChanged() {
	...
	return (currentAccount != null &&
	        !mOriginalCurrentAccount.equals(currentAccount.name));
}
...
/**
 * creates the account list items list including the add-account action in
 * case multiaccount_support is enabled.
 *
...
private ArrayList<AccountListItem> getAccountListItems() {
	Account[] accountList = AccountManager.get(this).getAccountsByType(
		MainApp.Companion.getAccountType());
	ArrayList<AccountListItem> adapterAccountList =
		new ArrayList<>(accountList.length);
	for (Account account : accountList) {
		adapterAccountList.add(new AccountListItem(account));
	}

	// Add Create Account item at the end of account list if multi-account is
	// enabled
...
public void removeAccount(final Account account) {
	...
	RemoveAccountDialogFragment dialog =
		RemoveAccountDialogFragment.newInstance(account);
	dialog.show(getSupportFragmentManager(),
	            RemoveAccountDialogFragment.FTAG_CONFIRMATION);
}

@Override
public void changePasswordOfAccount(final Account account) {
	Intent updateAccountCredentials =
		new Intent(ManageAccountsActivity.this, LoginActivity.class);
...
public void createAccount() {
	...
	am.addAccount(
		MainApp.Companion.getAccountType(), null, null, null,
			        ...
			        if (future != null) {
			                try {
			                        ...
			                        AccountUtils.setCurrentOwnCloudAccount(getApplicationContext(),
			                                                               name);
			                        mAccountListAdapter =
							new AccountListAdapter(ManageAccountsActivity.this,
 ...
 *
 * @param future Result of the removal; future.getResult() is true if account
...
public void run(final AccountManagerFuture<Boolean> future) {
	if (future != null && future.isDone()) {
		Account account =
			new Account(mAccountBeingRemoved, MainApp.Companion.getAccountType());
		if (!AccountUtils.exists(account.name,
		...
		if (am.getAccountsByType(MainApp.Companion.getAccountType()).length ==
		    0) {
			// Show create account screen if there isn't any account
			am.addAccount(MainApp.Companion.getAccountType(), null, null, null,
			              this, null, null);
...
/**
 * Switch current account to that contained in the received position of the
 * list adapter.
 *
...
private void switchAccount(final int position) {
	...
	if (getAccount().name.equals(clickedAccount.name)) {
		...
		AccountUtils.setCurrentOwnCloudAccount(ManageAccountsActivity.this,
		                                       clickedAccount.name);
		// Refresh dependencies to be used in selected account
		MainApp.Companion.initDependencyInjection();
		Intent i =
			new Intent(ManageAccountsActivity.this, FileDisplayActivity.class);
...
@Override
public OperationsService.OperationsServiceBinder
...
public void onServiceConnected(final ComponentName component,
	...
	if (component.equals(new ComponentName(ManageAccountsActivity.this,
	                                       FileDownloader.class))) {
		mDownloaderBinder = (FileDownloader.FileDownloaderBinder)service;

	} else if (component.equals(new ComponentName(ManageAccountsActivity.this,
	                                              FileUploader.class))) {
