...
public class PassCodeActivity extends BaseActivity {
  ...
  public final static String ACTION_REQUEST_WITH_RESULT =
      "ACTION_REQUEST_WITH_RESULT";
  public final static String ACTION_CHECK_WITH_RESULT =
  ...
  // NOTE: PREFERENCE_SET_PASSCODE must have the same value as
  // preferences.xml-->android:key for passcode preference
   ...
   *
   * @param savedInstanceState    Previously saved state - irrelevant in this
   *     case
   */
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    if (ACTION_CHECK.equals(getIntent().getAction())) {
      ...
      if (savedInstanceState != null) {
        mConfirmingPassCode = savedInstanceState.getBoolean(
            PassCodeActivity.KEY_CONFIRMING_PASSCODE);
        mPassCodeDigits = savedInstanceState.getStringArray(
            PassCodeActivity.KEY_PASSCODE_DIGITS);
      }
      ...
      throw new IllegalArgumentException(
          R.string.illegal_argument_exception_message + " ");
    }
  ...
  /**
   * Enables or disables the cancel button to allow the user interrupt the
   * ACTION requested to the activity.
   *
   * @param enabled       'True' makes the cancel button available, 'false'
   *     hides it.
   */
  ...
  protected void setTextListeners() {
    ...
    ///  First input field
    mPassCodeEditTexts[0].addTextChangedListener(
        new PassCodeDigitTextWatcher(0, false));

    /*------------------------------------------------
     *  SECOND BOX
     -------------------------------------------------*/
    mPassCodeEditTexts[1].addTextChangedListener(
        new PassCodeDigitTextWatcher(1, false));
    ...
    mPassCodeEditTexts[1].setOnKeyListener(new View.OnKeyListener() {
      ...
      public boolean onKey(final View v, final int keyCode,
        ...
        if (keyCode == KeyEvent.KEYCODE_DEL &&
            mBChange) { // TODO WIP: event should be
          // used to control what's exactly happening with DEL, not any custom
          // field...
    ...
    mPassCodeEditTexts[1].setOnFocusChangeListener(
        new View.OnFocusChangeListener() {
          ...
          public void onFocusChange(final View v, final boolean hasFocus) {
            ...
            if (mPassCodeEditTexts[0].getText().toString().equals(
                    "")) { // TODO WIP validation
              // could be done in a global way, with a single
              // OnFocusChangeListener for all the input fields
              mPassCodeEditTexts[0].requestFocus();
            }
          }
        });
     ...
     *  THIRD BOX
     -------------------------------------------------*/
    mPassCodeEditTexts[2].addTextChangedListener(
        new PassCodeDigitTextWatcher(2, false));
    ...
    mPassCodeEditTexts[2].setOnFocusChangeListener(
        new View.OnFocusChangeListener() {
          @Override
          public void onFocusChange(final View v, final boolean hasFocus) {
            if (mPassCodeEditTexts[0].getText().toString().equals("")) {
     ...
     *  FOURTH BOX
     -------------------------------------------------*/
    mPassCodeEditTexts[3].addTextChangedListener(
        new PassCodeDigitTextWatcher(3, true));
    ...
    mPassCodeEditTexts[3].setOnFocusChangeListener(
        new View.OnFocusChangeListener() {
          @Override
          public void onFocusChange(final View v, final boolean hasFocus) {
  ...
  /**
   * Processes the pass code entered by the user just after the last digit was
   * in.
   *
   * Takes into account the action requested to the activity, the currently
   * saved pass code and the previously typed pass code, if any.
   */
    ...
    if (ACTION_CHECK.equals(getIntent().getAction())) {
      if (checkPassCode()) {
        ...
        showErrorAndRestart(R.string.pass_code_wrong,
                            R.string.pass_code_enter_pass_code, View.INVISIBLE);
      }
      ...
      if (checkPassCode()) {
        ...
        hideSoftKeyboard();
        DocumentProviderUtils.Companion.notifyDocumentProviderRoots(
  ...
  private void showErrorAndRestart(final int errorMessage,
    ...
    Snackbar snackbar = Snackbar.make(findViewById(android.R.id.content),
                                      errorSeq, Snackbar.LENGTH_LONG);
    snackbar.show();
    mPassCodeHdr.setText(headerMessage); // TODO check if really needed
    mPassCodeHdrExplanation.setVisibility(
        explanationVisibility); // TODO check if really needed
    clearBoxes();
  }

  /**
   * Ask to the user for retyping the pass code just entered before saving it as
   * the current pass code.
   */
  ...
  /**
   * Compares pass code entered by the user with the value currently saved in
   * the app.
   *
  ...
  /**
   * Compares pass code retyped by the user in the input fields with the value
   * entered just before.
   *
  ...
  protected boolean confirmPassCode() {
    ...
    for (int i = 0; i < mPassCodeEditTexts.length && result; i++) {
      result = ((mPassCodeEditTexts[i].getText().toString())
                    .equals(mPassCodeDigits[i]));
    }
    return result;
  }
  ...
  /**
   * Overrides click on the BACK arrow to correctly cancel ACTION_ENABLE or
   * ACTION_DISABLE, while preventing than ACTION_CHECK may be worked around.
   *
   * @param keyCode       Key code of the key that triggered the down event.
   * @param event         Event triggered.
   * @return              'True' when the key event was processed by this
   *     method.
   */
  @Override
  ...
  protected void savePassCodeAndExit() {
    ...
    resultIntent.putExtra(KEY_PASSCODE,
                          mPassCodeDigits[0] + mPassCodeDigits[1] +
                              mPassCodeDigits[2] + mPassCodeDigits[3]);
  ...
  public void onSaveInstanceState(final Bundle outState) {
    ...
    outState.putBoolean(PassCodeActivity.KEY_CONFIRMING_PASSCODE,
                        mConfirmingPassCode);
    outState.putStringArray(PassCodeActivity.KEY_PASSCODE_DIGITS,
                            mPassCodeDigits);
  }
  ...
  private class PassCodeDigitTextWatcher implements TextWatcher {
     ...
     * Constructor
     *
     * @param index         Position in the pass code of the input field that
     *     will be bound to
     *                      this watcher.
     * @param lastOne       'True' means that watcher corresponds to the last
     *     position of the
     *                      pass code.
     */
    public PassCodeDigitTextWatcher(final int index, final boolean lastOne) {
      ...
      if (mIndex < 0) {
        throw new IllegalArgumentException(
            "Invalid index in " +
            PassCodeDigitTextWatcher.class.getSimpleName() + " constructor");
      }
    }

    private int next() { return mLastOne ? 0 : mIndex + 1; }

    /**
     * Performs several actions when the user types a digit in an input field:
     *  - saves the input digit to the state of the activity; this will allow
     * retyping the pass code to confirm it.
    ...
    public void afterTextChanged(final Editable s) {
      if (s.length() > 0) {
        if (!mConfirmingPassCode) {
          mPassCodeDigits[mIndex] =
              mPassCodeEditTexts[mIndex].getText().toString();
        }
