 ...
 * <p>
 * It proxies the necessary calls via {@link
 * androidx.appcompat.app.AppCompatDelegate} to be used with AppCompat.
 */
...
private static final String PREFERENCE_CAMERA_PICTURE_UPLOADS =
	"camera_picture_uploads";
private static final String PREFERENCE_CAMERA_PICTURE_UPLOADS_PATH =
	"camera_picture_uploads_path";
private static final String PREFERENCE_CAMERA_UPLOADS_CATEGORY =
	"camera_uploads_category";
private static final String PREFERENCE_CAMERA_PICTURE_UPLOADS_ON_WIFI =
	"camera_picture_uploads_on_wifi";
private static final String PREFERENCE_CAMERA_VIDEO_UPLOADS =
	"camera_video_uploads";
private static final String PREFERENCE_CAMERA_VIDEO_UPLOADS_PATH =
	"camera_video_uploads_path";
private static final String PREFERENCE_CAMERA_VIDEO_UPLOADS_ON_WIFI =
	"camera_video_uploads_on_wifi";
private static final String PREFERENCE_CAMERA_UPLOADS_SOURCE_PATH =
	"camera_uploads_source_path";
private static final String PREFERENCE_CAMERA_UPLOADS_BEHAVIOUR =
	"camera_uploads_behaviour";
private static final String PREFERENCE_SECURITY_CATEGORY =
	"security_category";
private static final String PREFERENCE_MORE_CATEGORY = "more";
private static final String PREFERENCE_HELP = "help";
private static final String PREFERENCE_SYNC_CALENDAR_CONTACTS =
...
public static final String PREFERENCE_TOUCHES_WITH_OTHER_VISIBLE_WINDOWS =
	"touches_with_other_visible_windows";
...
public void onCreate(final Bundle savedInstanceState) {
	...
	final String appVersion =
		temp + " " + BuildConfig.BUILD_TYPE + " " + BuildConfig.COMMIT_SHA1;
	...
	mPrefCameraPictureUploads =
		(CheckBoxPreference)findPreference(PREFERENCE_CAMERA_PICTURE_UPLOADS);

	mPrefCameraPictureUploadsPath =
		findPreference(PREFERENCE_CAMERA_PICTURE_UPLOADS_PATH);
	...
	mPrefCameraUploadsCategory =
		(PreferenceCategory)findPreference(PREFERENCE_CAMERA_UPLOADS_CATEGORY);

	mPrefCameraPictureUploadsWiFi =
		findPreference(PREFERENCE_CAMERA_PICTURE_UPLOADS_ON_WIFI);

	toggleCameraUploadsPictureOptions(true,
	                                  mPrefCameraPictureUploads.isChecked());

	mPrefCameraPictureUploads.setOnPreferenceChangeListener(
		(preference, newValue)->{
			boolean enableCameraUploadsPicture = (Boolean)newValue;
			toggleCameraUploadsPictureOptions(false, enableCameraUploadsPicture);
			toggleCameraUploadsCommonOptions(mPrefCameraVideoUploads.isChecked(),
	...
	mPrefCameraVideoUploads =
		(CheckBoxPreference)findPreference(PREFERENCE_CAMERA_VIDEO_UPLOADS);

	mPrefCameraVideoUploadsPath =
		findPreference(PREFERENCE_CAMERA_VIDEO_UPLOADS_PATH);
	...
	mPrefCameraVideoUploadsWiFi =
		findPreference(PREFERENCE_CAMERA_VIDEO_UPLOADS_ON_WIFI);
	toggleCameraUploadsVideoOptions(true, mPrefCameraVideoUploads.isChecked());

	mPrefCameraVideoUploads.setOnPreferenceChangeListener(
		(preference, newValue)->{
	...
	mPrefCameraUploadsSourcePath =
		findPreference(PREFERENCE_CAMERA_UPLOADS_SOURCE_PATH);
	if (mPrefCameraUploadsSourcePath != null) {
		mPrefCameraUploadsSourcePath.setOnPreferenceClickListener(preference->{
				...
				LocalFolderPickerActivity.startLocalFolderPickerActivityForResult(
					Preferences.this, mSourcePath, ACTION_SELECT_SOURCE_PATH);
				return true;
			});
	} else {
	...
	mPrefCameraUploadsBehaviour =
		findPreference(PREFERENCE_CAMERA_UPLOADS_BEHAVIOUR);
	toggleCameraUploadsCommonOptions(mPrefCameraVideoUploads.isChecked(),
	                                 mPrefCameraPictureUploads.isChecked());
	...
	CameraUploadsConfiguration configuration =
		com.owncloud.android.db.PreferenceManager.getCameraUploadsConfiguration(
			this);
	...
	mPrefSecurityCategory =
		(PreferenceCategory)findPreference(PREFERENCE_SECURITY_CATEGORY);
	mPasscode = (CheckBoxPreference)findPreference(
		PassCodeActivity.PREFERENCE_SET_PASSCODE);
	mPattern = (CheckBoxPreference)findPreference(
		PatternLockActivity.PREFERENCE_SET_PATTERN);
	mBiometric = (CheckBoxPreference)findPreference(
		BiometricActivity.PREFERENCE_SET_BIOMETRIC);
	mPrefTouchesWithOtherVisibleWindows = (CheckBoxPreference)findPreference(
	...
	if (mPasscode != null) {
		mPasscode.setOnPreferenceChangeListener((preference, newValue)->{
				...
				patternSet = mAppPrefs.getBoolean(
	...
	if (mPattern != null) {
		mPattern.setOnPreferenceChangeListener((preference, newValue)->{
				Intent intent =
					new Intent(getApplicationContext(), PatternLockActivity.class);
				Boolean state = (Boolean)newValue;
				passcodeSet = mAppPrefs.getBoolean(
					PassCodeActivity.PREFERENCE_SET_PASSCODE, false);
				if (passcodeSet) {
				        showSnackMessage(R.string.passcode_already_set);
				} else {
				        intent.setAction(state
			       ? PatternLockActivity.ACTION_REQUEST_WITH_RESULT
	...
	if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
		...
		if (mPasscode != null && mPattern != null && !mPasscode.isChecked() &&
		    !mPattern.isChecked()) {
		...
		mBiometric.setOnPreferenceChangeListener((preference, newValue)->{
				...
				if (incoming && mBiometricManager != null &&
				    !mBiometricManager.isHardwareDetected()) {
		...
		mPrefTouchesWithOtherVisibleWindows.setOnPreferenceChangeListener(
			(preference, newValue)->{
				...
				if ((Boolean)newValue) {
				        ...
				        .setTitle(getString(
							  R.string.confirmation_touches_with_other_windows_title))
				        .setMessage(getString(
							    R.string.confirmation_touches_with_other_windows_message))
	 ...
	 * More
	 */
	PreferenceCategory pCategoryMore =
		(PreferenceCategory)findPreference(PREFERENCE_MORE_CATEGORY);
	...
	Preference pSyncCalendarContacts =
		findPreference(PREFERENCE_SYNC_CALENDAR_CONTACTS);

	boolean syncCalendarContactsEnabled =
		getResources().getBoolean(R.bool.sync_calendar_contacts_enabled);
	if (pSyncCalendarContacts != null) {
		if (syncCalendarContactsEnabled) {
			pSyncCalendarContacts.setOnPreferenceClickListener(preference->{
					String syncCalendarContactsUrl =
						(String)getText(R.string.url_sync_calendar_contacts);
	...
	boolean recommendEnabled =
		getResources().getBoolean(R.bool.recommend_enabled);
		...
		if (recommendEnabled) {
			pRecommend.setOnPreferenceClickListener(preference->{
					...
					String recommendText = String.format(
						getString(R.string.recommend_text), appName, downloadUrl);
	...
	boolean feedbackEnabled =
		getResources().getBoolean(R.bool.feedback_enabled);
	Preference pFeedback = findPreference(PREFERENCE_FEEDBACK);
	if (pFeedback != null) {
		if (feedbackEnabled) {
			pFeedback.setOnPreferenceClickListener(preference->{
					String feedbackMail = (String)getText(R.string.mail_feedback);
					String feedback = "Android v" + BuildConfig.VERSION_NAME + " - " +
					                  getText(R.string.prefs_feedback);
					Intent intent = new Intent(Intent.ACTION_SENDTO);
	...
	boolean privacyPolicyEnabled =
		getResources().getBoolean(R.bool.privacy_policy_enabled);
	...
	mLogger.setOnPreferenceClickListener(preference->{
			Intent loggerIntent =
				new Intent(getApplicationContext(), LogHistoryActivity.class);
	...
	if (mAboutApp != null) {
		mAboutApp.setTitle(String.format(getString(R.string.about_android),
		                                 getString(R.string.app_name)));
		mAboutApp.setSummary(
			String.format(getString(R.string.about_version), appVersion));
		mAboutApp.setOnPreferenceClickListener(preference->{
				int clickCount = mAppPrefs.getInt(MainApp.CLICK_DEV_MENU, 0);
				if (mAppPrefs.getInt(MainApp.CLICK_DEV_MENU, 0) >
				...
				.startLogIfDeveloper(); // read value to global variable
 ...
 *
 * @param initializing avoid showing the dialog to confirm camera uploads by
 *     disabling it in the first load of the
 *                     view and showing it when the user just unchecked the
 * feature checkbox
 * @param isChecked    camera uploads for pictures is checked
 */
private void toggleCameraUploadsPictureOptions(final Boolean initializing,
	...
	if (isChecked) {
		...
		if (!initializing) {
			showSimpleDialog(
				getString(R.string.proper_pics_folder_warning_camera_upload));
		}
	} else {
		if (!initializing) {
			new AlertDialog.Builder(this)
			...
			.setMessage(getString(
					    R.string.confirmation_disable_pictures_upload_message))
			.setNegativeButton(getString(R.string.common_no),
			                   (dialog, which)->{
					mPrefCameraPictureUploads.setChecked(true);
					mPrefCameraUploadsCategory.addPreference(
						mPrefCameraPictureUploadsWiFi);
					mPrefCameraUploadsCategory.addPreference(
						mPrefCameraPictureUploadsPath);
				})
			...
			.setPositiveButton(getString(R.string.common_yes),
					...
					mPrefCameraUploadsCategory.removePreference(
						mPrefCameraPictureUploadsWiFi);
					mPrefCameraUploadsCategory.removePreference(
						mPrefCameraPictureUploadsPath);
					mCameraUploadsHandler.updatePicturesLastSync(
 ...
 *
 * @param initializing to avoid showing the dialog to confirm camera uploads
 *     disabling in the first load of the
 *                     view and showing it when the user just unchecked the
 * feature checkbox
 * @param isChecked    camera uploads for videos is checked
 */
private void toggleCameraUploadsVideoOptions(final Boolean initializing,
                                             final Boolean isChecked) {
	if (isChecked) {
		...
		if (!initializing) {
			showSimpleDialog(
				getString(R.string.proper_videos_folder_warning_camera_upload));
		}
	} else {
		if (!initializing) {
			new AlertDialog.Builder(this)
			...
			.setPositiveButton(getString(R.string.common_yes),
					...
					mPrefCameraUploadsCategory.removePreference(
						mPrefCameraVideoUploadsPath);
					mCameraUploadsHandler.updateVideosLastSync(
						getApplicationContext(), 0);
				})
			.show();
...
private void toggleCameraUploadsCommonOptions(final Boolean video,
...
protected void onResume() {
	super.onResume();
	boolean passCodeState =
		mAppPrefs.getBoolean(PassCodeActivity.PREFERENCE_SET_PASSCODE, false);
	mPasscode.setChecked(passCodeState);
	boolean patternState =
		mAppPrefs.getBoolean(PatternLockActivity.PREFERENCE_SET_PATTERN, false);
	mPattern.setChecked(patternState);
	boolean biometricState =
		mAppPrefs.getBoolean(BiometricActivity.PREFERENCE_SET_BIOMETRIC, false);
...
protected void onActivityResult(final int requestCode, final int resultCode,
	...
	if (requestCode == ACTION_SELECT_UPLOAD_PATH && resultCode == RESULT_OK) {
	...
	} else if (requestCode == ACTION_SELECT_UPLOAD_VIDEO_PATH &&
	...
	} else if (requestCode == ACTION_SELECT_SOURCE_PATH &&
	           resultCode == RESULT_OK) {
		...
		if (previousSourcePath.endsWith(File.separator)) {
			previousSourcePath =
				previousSourcePath.substring(0, previousSourcePath.length() - 1);
		}

		if (!previousSourcePath.equals(
			    data.getStringExtra(LocalFolderPickerActivity.EXTRA_PATH))) {
			long currentTimeStamp = System.currentTimeMillis();
			mCameraUploadsHandler.updatePicturesLastSync(getApplicationContext(),
			                                             currentTimeStamp);
			mCameraUploadsHandler.updateVideosLastSync(getApplicationContext(),
			                                           currentTimeStamp);
		}
	...
	} else if (requestCode == ACTION_REQUEST_PASSCODE &&
	           resultCode == RESULT_OK) { // Enable passcode
		...
		if (passcode != null && passcode.length() == 4) {
			...
			for (int i = 1; i <= 4; ++i) {
				editor.putString(PassCodeActivity.PREFERENCE_PASSCODE_D + i,
			...
			enableBiometric();
	           ...
	           resultCode == RESULT_OK) { // Disable passcode
		...
		if (data.getBooleanExtra(PassCodeActivity.KEY_CHECK_RESULT, false)) {
			...
			// Do not allow to use biometric lock since Passcode lock has been
			// disabled
			disableBiometric(getString(R.string.prefs_biometric_summary));
		}
	} else if (requestCode == ACTION_REQUEST_PATTERN &&
	           resultCode == RESULT_OK) { // Enable pattern
		String patternValue =
...
protected void onStop() {
	...
	if (configuration.isEnabledForPictures() ||
	    configuration.isEnabledForVideos()) {
...
private void saveCameraUploadsPicturePathOnPreferences() {
	mAppPrefs.edit()
	.putString(PREFERENCE_CAMERA_PICTURE_UPLOADS_PATH, mUploadPath)
	.apply();
}
...
private void saveCameraUploadsVideoPathOnPreferences() {
	mAppPrefs.edit()
	.putString(PREFERENCE_CAMERA_VIDEO_UPLOADS_PATH, mUploadVideoPath)
	.apply();
}
...
private void loadCameraUploadsSourcePath() {
	mSourcePath =
		mAppPrefs.getString(PREFERENCE_CAMERA_UPLOADS_SOURCE_PATH,
	...
	if (mPrefCameraUploadsSourcePath != null) {
		...
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
			comment =
				getString(R.string.prefs_camera_upload_source_path_title_optional);
		} else {
			comment =
				getString(R.string.prefs_camera_upload_source_path_title_required);
		}
		mPrefCameraUploadsSourcePath.setTitle(String.format(
							      mPrefCameraUploadsSourcePath.getTitle().toString(), comment));
	}
}
...
private void saveCameraUploadsSourcePathOnPreferences() {
	mAppPrefs.edit()
	.putString(PREFERENCE_CAMERA_UPLOADS_SOURCE_PATH, mSourcePath)
	.apply();
}
