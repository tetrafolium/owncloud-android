...
public class ReceiveExternalFilesActivity extends FileActivity
  ...
  private final static int REQUEST_CODE__SETUP_ACCOUNT =
  ...
  private final static String KEY_ACCOUNT_SELECTION_SHOWING =
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    if (savedInstanceState == null) {
      ...
      mAccountSelectionShowing =
          savedInstanceState.getBoolean(KEY_ACCOUNT_SELECTION_SHOWING);
    }
    ...
    syncIntentFilter.addAction(
        RefreshFolderOperation.EVENT_SINGLE_FOLDER_SHARES_SYNCED);
    mSyncBroadcastReceiver = new SyncBroadcastReceiver();
    mLocalBroadcastManager = LocalBroadcastManager.getInstance(this);
    mLocalBroadcastManager.registerReceiver(mSyncBroadcastReceiver,
                                            syncIntentFilter);

    // Listen for upload messages
    IntentFilter uploadIntentFilter =
        new IntentFilter(FileUploader.getUploadFinishMessage());
    uploadIntentFilter.addAction(FileUploader.getUploadStartMessage());
    mUploadBroadcastReceiver = new UploadBroadcastReceiver();
    mLocalBroadcastManager.registerReceiver(mUploadBroadcastReceiver,
                                            uploadIntentFilter);
  ...
  protected void setAccount(final Account account, final boolean savedAccount) {
    if (somethingToUpload()) {
      ...
      Account[] accounts =
          mAccountManager.getAccountsByType(MainApp.Companion.getAccountType());
  ...
  protected void onSaveInstanceState(final Bundle outState) {
    ...
    outState.putBoolean(KEY_ACCOUNT_SELECTION_SHOWING,
                        mAccountSelectionShowing);
      ...
      builder.setPositiveButton(
          ...
          new OnClickListener() {
            ...
            public void onClick(final DialogInterface dialog, final int which) {
              ...
              intent.putExtra(
                  "authorities",
                  new String[] {MainApp.Companion.getAuthTokenType()});
    ...
    case DIALOG_MULTIPLE_ACCOUNT:
      ...
      for (int i = 0; i < dialogItems.length; ++i) {
        try {
          ...
          dialogItems[i] = oca.getDisplayName() + " @ " +
                           DisplayUtils.convertIdn(
                               accounts[i].name.substring(
                                   accounts[i].name.lastIndexOf("@") + 1),
                               false);
      ...
      builder.setItems(dialogItems, (dialog, which) -> {
        setAccount(mAccountManager.getAccountsByType(
            MainApp.Companion.getAccountType())[which]);
  ...
  protected void onActivityResult(final int requestCode, final int resultCode,
    ...
    if (requestCode == REQUEST_CODE__SETUP_ACCOUNT) {
      ...
      Account[] accounts = mAccountManager.getAccountsByType(
          MainApp.Companion.getAuthTokenType());
      if (accounts.length == 0) {
        showDialog(DIALOG_NO_ACCOUNT);
      } else {
  ...
  private void startSyncFolderOperation(final OCFile folder) {
    ...
    SyncOperation synchFolderOp = new RefreshFolderOperation(
        folder, false, getAccount(), getApplicationContext());
  ...
  /**
   * Checks if the intent contains plain text and no other stream has been added
   * yet.
   *
   * @return true/false
   */
  ...
  public void uploadFiles() {
    ...
    UriUploader uploader =
        new UriUploader(this, mStreamsToUpload, mUploadPath, getAccount(),
                        FileUploader.LOCAL_BEHAVIOUR_FORGET,
                        true, // Show waiting dialog while file is being copied
    ...
    // Save the path to shared preferences; even if upload is not possible, user
    // chose the folder
    ...
    if (resultCode == UriUploader.UriUploaderResultCode.OK) {
      ...
      if (resultCode ==
          UriUploader.UriUploaderResultCode.ERROR_NO_FILE_TO_UPLOAD) {
      ...
      } else if (resultCode == UriUploader.UriUploaderResultCode
   ...
   */
  private void
  onCreateFolderOperationFinish(final CreateFolderOperation operation,
                                final RemoteOperationResult result) {
    if (result.isSuccess()) {
      populateDirectoryList();
    } else {
      try {
        showSnackMessage(ErrorMessageAdapter.Companion.getResultMessage(
            result, operation, getResources()));
  ...
  private void initTargetFolder() {
    ...
    // Make sure that path still exists, if it doesn't pop the stack and try the
    // previous path
    while (!getStorageManager().fileExists(generatePath(mParents)) &&
           mParents.size() > 1) {
      mParents.pop();
    }
  }
  ...
  public boolean onCreateOptionsMenu(final Menu menu) {
    ...
    inflater.inflate(R.menu.sort_menu,
                     menu.findItem(R.id.action_sort).getSubMenu());
  ...
  public boolean onOptionsItemSelected(final MenuItem item) {
    ...
    case R.id.action_sort_descending:
      ...
      PreferenceManager.setSortAscending(isAscending, this,
                                         FileStorageUtils.FILE_DISPLAY_SORT);
      switch (PreferenceManager.getSortOrder(
          this, FileStorageUtils.FILE_DISPLAY_SORT)) {
    ...
    case R.id.action_sort_by_name:
      ...
      sortByName(PreferenceManager.getSortAscending(
          this, FileStorageUtils.FILE_DISPLAY_SORT));
      break;
    case R.id.action_sort_by_size:
      item.setChecked(true);
      sortBySize(PreferenceManager.getSortAscending(
          this, FileStorageUtils.FILE_DISPLAY_SORT));
      break;
    case R.id.action_sort_by_date:
      item.setChecked(true);
      sortByDate(PreferenceManager.getSortAscending(
          this, FileStorageUtils.FILE_DISPLAY_SORT));
      break;
    default:
  ...
  private void recoverSortMenuState(final Menu menu) {
    if (menu != null) {
      menu.findItem(R.id.action_sort_descending)
          .setChecked(!PreferenceManager.getSortAscending(
              this, FileStorageUtils.FILE_DISPLAY_SORT));
      switch (PreferenceManager.getSortOrder(
          this, FileStorageUtils.FILE_DISPLAY_SORT)) {
      case FileStorageUtils.SORT_NAME:
  ...
  private class SyncBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
      ...
      boolean sameAccount =
          (getAccount() != null && accountName.equals(getAccount().name) &&
      ...
      if (sameAccount) {
        ...
        if (FileSyncAdapter.EVENT_FULL_SYNC_START.equals(event)) {
          ...
          OCFile currentDir = (getCurrentFolder() == null)
                                  ? null
                                  : getStorageManager().getFileByPath(
                                        getCurrentFolder().getRemotePath());
          ...
          mSyncInProgress =
              (!FileSyncAdapter.EVENT_FULL_SYNC_END.equals(event) &&
               !RefreshFolderOperation.EVENT_SINGLE_FOLDER_SHARES_SYNCED.equals(
                   event));

          if (RefreshFolderOperation.EVENT_SINGLE_FOLDER_CONTENTS_SYNCED.equals(
                  event) && /// TODO refactor and make common
              synchResult != null &&
  ...
  /**
   * Show an error dialog, forcing the user to click a single button to exit the
   * activity
   *
   * @param messageResId    DataResult id of the message to show in the dialog.
   * @param messageResTitle DataResult id of the title to show in the dialog. 0
   *     to show default alert message.
   *                        -1 to show no title.
   */
  private void showErrorDialog(final int messageResId,
                               final int messageResTitle) {

    ConfirmationDialogFragment errorDialog =
        ConfirmationDialogFragment.newInstance(
            messageResId,
            new String[] {getString(
                R.string
                    .app_name)}, // see uploader_error_message_* in strings.xml
            messageResTitle, R.string.common_back, -1, -1);
  ...
  /**
   * Show a dialog where the user can enter a filename for the file he wants to
   * place the text in.
   */
  private void showUploadTextDialog() {
    ...
    final View dialogView =
        getLayoutInflater().inflate(R.layout.dialog_upload_text, null);
    ...
    final TextInputLayout inputLayout =
        dialogView.findViewById(R.id.inputTextLayout);
      ...
      button.setOnClickListener(view -> {
        ...
        if (fileName.length() > MAX_FILENAME_LENGTH) {
          error = String.format(
              getString(
                  R.string
                      .uploader_upload_text_dialog_filename_error_length_max),
              MAX_FILENAME_LENGTH);
        } else if (fileName.length() == 0) {
          error = getString(
              R.string.uploader_upload_text_dialog_filename_error_empty);
        } else {
          fileName += ".txt";
  ...
  private Uri savePlainTextToFile(final String fileName) {
    ...
    try {
      ...
      Timber.w(e, "Failed to create temp file for uploading plain text: %s",
               e.getMessage());
    }
    return uri;
  }
   ...
   */
  private void setFileNameFromIntent(final AlertDialog alertDialog,
  ...
  private class UploadBroadcastReceiver extends BroadcastReceiver {
    /**
     * If the upload is text shared from other apps and was successfully
     * uploaded -> delete cache
     */
    @Override
    public void onReceive(final Context context, final Intent intent) {
      boolean success =
          intent.getBooleanExtra(Extras.EXTRA_UPLOAD_RESULT, false);
