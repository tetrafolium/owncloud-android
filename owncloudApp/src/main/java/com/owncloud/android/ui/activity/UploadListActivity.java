 ...
 */
public class UploadListActivity
	extends FileActivity implements UploadListFragment.ContainerActivity {
...
private void createUploadListFragment() {
	...
	transaction.add(R.id.left_fragment_container, uploadList,
	                TAG_UPLOAD_LIST_FRAGMENT);
	transaction.commit();
}
...
protected void onResume() {
	...
	mLocalBroadcastManager.registerReceiver(mUploadMessagesReceiver,
	                                        uploadIntentFilter);
...
/**
 * Open file with app associates with its MIME type. If MIME type unknown,
 * show list with all apps.
 */
...
public boolean onOptionsItemSelected(final MenuItem item) {
	...
	UploadListFragment uploadListFragment =
		(UploadListFragment)getSupportFragmentManager().findFragmentByTag(
			TAG_UPLOAD_LIST_FRAGMENT);
...
protected void onActivityResult(final int requestCode, final int resultCode,
	...
	if (requestCode == FileActivity.REQUEST_CODE__UPDATE_CREDENTIALS &&
...
public void onRemoteOperationFinish(final RemoteOperation operation,
	...
	if (operation instanceof CheckCurrentCredentialsOperation) {
		// Do not call super in this case; more refactoring needed around
		// onRemoteOeprationFinish :'(
		...
		if (!result.isSuccess()) {
			...
			TransferRequester requester = new TransferRequester();
			requester.retryFailedUploads(this, account,
			                             UploadResult.CREDENTIAL_ERROR, false);
		}
...
public void onServiceDisconnected(final ComponentName component) {
	if (component.equals(
		    new ComponentName(UploadListActivity.this, FileUploader.class))) {
		Timber.d(
			"UploadListActivity suddenly disconnected from Upload service");
