...
// public DiskLruImageCache( Context context,String uniqueName, int
// diskCacheSize,
public DiskLruImageCache(final File diskCacheDir, final int diskCacheSize,
                         final CompressFormat compressFormat,
                         final int quality) throws IOException {

	mDiskCache = DiskLruCache.open(diskCacheDir, CACHE_VERSION, VALUE_COUNT,
	                               diskCacheSize);
...
public void put(final String key, final Bitmap data) {
	...
	try {
		...
		if (writeBitmapToFile(data, editor)) {
			...
			editor.abort();
			if (MainApp.Companion.isDeveloper()) {
				Timber.d("cache_test_DISK_ ERROR on: image put on disk cache %s",
				         validKey);
			}
		}
	} catch (IOException e) {
...
public Bitmap getBitmap(final String key) {
	...
	if (MainApp.Companion.isDeveloper()) {
		Timber.d(bitmap == null ? "not found" : "image read from disk %s",
		         validKey);
	}
