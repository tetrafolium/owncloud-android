 ...
 *
 */
public class ExpandableUploadListAdapter
    extends BaseExpandableListAdapter implements Observer {
  ...
  abstract class UploadGroup implements Refresh {
    ...
    int getGroupCount() { return items == null ? 0 : items.length; }
    ...
    Comparator<OCUpload> comparator = new Comparator<OCUpload>() {
      ...
      public int compare(final OCUpload upload1, final OCUpload upload2) {
        if (upload1.getUploadStatus().equals(UploadStatus.UPLOAD_IN_PROGRESS)) {
          if (!upload2.getUploadStatus().equals(
                  UploadStatus.UPLOAD_IN_PROGRESS)) {
            return -1;
          }
          // both are in progress
          FileUploader.FileUploaderBinder binder =
              mParentActivity.getFileUploaderBinder();
          if (binder != null) {
        ...
        } else if (upload2.getUploadStatus().equals(
                       UploadStatus.UPLOAD_IN_PROGRESS)) {
          return 1;
        }
        if (upload1.getUploadEndTimestamp() == 0 ||
            upload2.getUploadEndTimestamp() == 0) {
      ...
      private int compareUploadId(final OCUpload upload1,
                                  final OCUpload upload2) {
        return Long.compare(upload1.getUploadId(), upload2.getUploadId());
      }

      private int compareUpdateTime(final OCUpload upload1,
                                    final OCUpload upload2) {
        return Long.compare(upload2.getUploadEndTimestamp(),
                            upload1.getUploadEndTimestamp());
      }
    };
  ...
  public ExpandableUploadListAdapter(final FileActivity parentActivity) {
    ...
    mUploadsStorageManager =
        new UploadsStorageManager(mParentActivity.getContentResolver());
    mUploadGroups = new UploadGroup[3];
    mUploadGroups[0] = new UploadGroup(mParentActivity.getString(
    ...
    mUploadGroups[1] = new UploadGroup(
        mParentActivity.getString(R.string.uploads_view_group_failed_uploads)) {
      @Override
      public void refresh() {
    ...
    mUploadGroups[2] = new UploadGroup(mParentActivity.getString(
        R.string.uploads_view_group_finished_uploads)) {
      @Override
      public void refresh() {
  ...
  private View getView(final OCUpload[] uploadsItems, final int position,
    ...
    if (uploadsItems != null && uploadsItems.length > position) {
      ...
      String pathText =
          mParentActivity.getString(R.string.app_name) + remoteParentPath;
      pathTextView.setText(pathText);

      // file size
      TextView fileSizeTextView = view.findViewById(R.id.upload_file_size);
      String fileSize = DisplayUtils.bytesToHumanReadable(upload.getFileSize(),
                                                          mParentActivity) +
                        ", ";
      ...
      CharSequence dateString = DisplayUtils.getRelativeDateTimeString(
          mParentActivity, updateTime, DateUtils.SECOND_IN_MILLIS,
      ...
      try {
        Account account = AccountUtils.getOwnCloudAccountByName(
            mParentActivity, upload.getAccountName());
      ...
      case UPLOAD_IN_PROGRESS:
        ...
        if (binder != null) {
          if (binder.isUploadingNow(upload)) {
            ...
            if (convertView != null && mProgressListener != null &&
          ...
          Timber.w("FileUploaderBinder not ready yet for upload %s",
      ...
      if (upload.getUploadStatus() == UploadStatus.UPLOAD_FAILED) {
        if (UploadResult.CREDENTIAL_ERROR.equals(upload.getLastResult())) {
          view.setOnClickListener(
              v
              -> mParentActivity.getFileOperationsHelper()
      ...
      /* Cancellation needs do be checked and done before changing the drawable
       * in fileIcon, or
       * {@link ThumbnailsCacheManager#cancelPotentialWork} will NEVER cancel
       * any task.
       */
      OCFile fakeFileToCheatThumbnailsCacheManagerInterface =
          new OCFile(upload.getRemotePath());
      fakeFileToCheatThumbnailsCacheManagerInterface.setStoragePath(
          upload.getLocalPath());
      fakeFileToCheatThumbnailsCacheManagerInterface.setMimetype(
          upload.getMimeType());

      boolean allowedToCreateNewThumbnail =
          (ThumbnailsCacheManager.cancelPotentialThumbnailWork(
              fakeFileToCheatThumbnailsCacheManagerInterface, fileIcon));

      // TODO this code is duplicated; refactor to a common place
      if ((fakeFileToCheatThumbnailsCacheManagerInterface.isImage() &&
           fakeFileToCheatThumbnailsCacheManagerInterface.getRemoteId() !=
               null &&
           upload.getUploadStatus() == UploadStatus.UPLOAD_SUCCEEDED)) {
        // Thumbnail in Cache?
        Bitmap thumbnail =
            ThumbnailsCacheManager.getBitmapFromDiskCache(String.valueOf(
                fakeFileToCheatThumbnailsCacheManagerInterface.getRemoteId()));
        if (thumbnail != null && !fakeFileToCheatThumbnailsCacheManagerInterface
          ...
          if (allowedToCreateNewThumbnail) {
            final ThumbnailsCacheManager.ThumbnailGenerationTask task =
                new ThumbnailsCacheManager.ThumbnailGenerationTask(
                    fileIcon, mParentActivity.getStorageManager(),
            ...
            final ThumbnailsCacheManager.AsyncThumbnailDrawable asyncDrawable =
                new ThumbnailsCacheManager.AsyncThumbnailDrawable(
                    mParentActivity.getResources(), thumbnail, task);
        ...
        if ("image/png".equals(upload.getMimeType())) {
          fileIcon.setBackgroundColor(mParentActivity.getResources().getColor(
        ...
        fileIcon.setImageResource(
            MimetypeIconUtil.getFileTypeIconId(upload.getMimeType(), fileName));
      }
    }
  ...
  /**
   * Gets the status text to show to the user according to the status and last
   * result of the the given upload.
   *
  ...
  private String getStatusText(final OCUpload upload) {
    ...
    case UPLOAD_IN_PROGRESS:
      ...
      if (binder != null && binder.isUploadingNow(upload)) {
        /// really uploading, bind the progress bar to listen for progress
        /// updates
        status = mParentActivity.getString(
    ...
    case UPLOAD_SUCCEEDED:
      status = mParentActivity.getString(
          R.string.uploads_view_upload_status_succeeded);
      break;
    ...
    case UPLOAD_FAILED:
      ...
      case QUOTA_EXCEEDED:
        status = mParentActivity.getString(
            R.string.failed_upload_quota_exceeded_text);
        break;
      ...
      case SPECIFIC_FORBIDDEN:
        // We don't know the specific forbidden error message because it is not
        // being saved in uploads storage
      ...
      case SPECIFIC_UNSUPPORTED_MEDIA_TYPE:
        // We don't know the specific unsupported media type error message
        // because it is not being saved in uploads storage
        status = mParentActivity.getString(
            R.string.uploads_view_unsupported_media_type);
        break;
      default:
        status =
  ...
  public View getChildView(final int groupPosition, final int childPosition,
    ...
    return getView(mUploadGroups[(int)getGroupId(groupPosition)].items,
  ...
  /**
   * Returns the groupId (that is, index in mUploadGroups) for group at position
   * groupPosition (0-based). Could probably be done more intuitive but this
   * tested methods works as intended.
   */
  @Override
  ...
  public View getGroupView(final int groupPosition, final boolean isExpanded,
    ...
    int stringResFileCount = group.getGroupCount() == 1
                                 ? R.string.uploads_view_group_file_count_single
                                 : R.string.uploads_view_group_file_count;
    String fileCountText = String.format(
        mParentActivity.getString(stringResFileCount), group.getGroupCount());
  ...
  public class ProgressListener implements OnDatatransferProgressListener {
    ...
    @Override
    public void onTransferProgress(final long progressRate,
    ...
    boolean isWrapping(final ProgressBar progressBar) {
      ...
      return (wrappedProgressBar != null &&
              wrappedProgressBar ==
                  progressBar // on purpose; don't replace with equals
      );
    }

    public OCUpload getUpload() { return mUpload; }
  }

  public void addBinder() { notifyDataSetChanged(); }
}
