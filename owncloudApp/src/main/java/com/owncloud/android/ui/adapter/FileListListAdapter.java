...
public class FileListListAdapter extends BaseAdapter implements ListAdapter {
  ...
  private Vector<OCFile> mImmutableFilesList =
      null; // List containing the database files, doesn't change with search
  private Vector<OCFile> mFiles =
      null; // List that can be changed when using search
  ...
  public FileListListAdapter(final boolean justFolders,
                             final boolean onlyAvailableOffline,
                             final boolean sharedByLinkFiles,
                             final boolean folderPicker, final Context context,
    ...
    FileStorageUtils.mSortOrderFileDisp = PreferenceManager.getSortOrder(
        mContext, FileStorageUtils.FILE_DISPLAY_SORT);
    FileStorageUtils.mSortAscendingFileDisp =
        PreferenceManager.getSortAscending(mContext,
  ...
  public boolean isEnabled(final int position) {
    // Disable click for files when selecting a folder in copying and moving
    // operations
  ...
  public View getView(final int position, final View convertView,
    ...
    if (file != null) {
      final ImageView localStateView =
          view.findViewById(R.id.localFileIndicator);
      ...
      case LIST_ITEM:
        ConstraintLayout constraintLayout =
            view.findViewById(R.id.file_list_constraint_layout);
        ...
        lastModTV.setText(DisplayUtils.getRelativeTimestamp(
      ...
      if (file.isFolder()) {
        // Folder
        fileIcon.setImageResource(MimetypeIconUtil.getFolderTypeIconId(
        ...
        if (file.isImage() && file.getRemoteId() != null) {
          // Thumbnail in Cache?
          Bitmap thumbnail =
              ThumbnailsCacheManager.getBitmapFromDiskCache(file.getRemoteId());
          if (thumbnail != null && !file.needsUpdateThumbnail()) {
            ...
            if (ThumbnailsCacheManager.cancelPotentialThumbnailWork(file,
              ...
              final ThumbnailsCacheManager
                  .AsyncThumbnailDrawable asyncDrawable =
  ...
  private void
  setIconPinAcordingToFilesLocalState(final ImageView localStateView,
                                      final OCFile file) {
    // local state
   ...
   */
  public void
  swapDirectory(final OCFile folder,
                final FileDataStorageManager updatedStorageManager) {
    if (updatedStorageManager != null &&
        updatedStorageManager != mStorageManager) {
      mStorageManager = updatedStorageManager;
      mAccount = AccountUtils.getCurrentOwnCloudAccount(mContext);
    }

    boolean isRootFolder =
        folder.equals(updatedStorageManager.getFileByPath(OCFile.ROOT_PATH));

    if (mStorageManager != null) {
      if (mOnlyAvailableOffline &&
          (isRootFolder || !folder.isAvailableOffline())) {
        mImmutableFilesList =
            updatedStorageManager.getAvailableOfflineFilesFromCurrentAccount();
      } else if (mSharedByLinkFiles && isRootFolder) {
        mImmutableFilesList =
            updatedStorageManager.getSharedByLinkFilesFromCurrentAccount();
      } else {
    ...
    mFiles =
        FileStorageUtils.sortFolder(mFiles, FileStorageUtils.mSortOrderFileDisp,
                                    FileStorageUtils.mSortAscendingFileDisp);
