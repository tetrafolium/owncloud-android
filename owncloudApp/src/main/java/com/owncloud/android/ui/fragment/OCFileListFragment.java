...
public class OCFileListFragment extends ExtendedListFragment
    implements SearchView.OnQueryTextListener, View.OnFocusChangeListener {

  private static final String MY_PACKAGE =
      OCFileListFragment.class.getPackage() != null
          ? OCFileListFragment.class.getPackage().getName()
          : "com.owncloud.android.ui.fragment";

  private final static String ARG_ALLOW_CONTEXTUAL_MODE =
      MY_PACKAGE + ".ALLOW_CONTEXTUAL";
   ...
   *
   * @param justFolders         When 'true', only folders will be shown to the
   *     user, not files.
   * @param fileListOption      File list option to show. All files by default.
   * @param pickingAFolder      When 'true', only folders will be clickable when
   *     selecting a folder when copying or
   *                            moving files or configuring upload path for
   * camera uploads
   * @param hideFAB             When 'true', floating action button is hidden.
   * @param allowContextualMode When 'true', contextual action mode is enabled
   *     long-pressing an item.
   * @return New fragment with arguments set.
   */
  public static OCFileListFragment
  newInstance(final boolean justFolders, final FileListOption fileListOption,
              final boolean pickingAFolder, final boolean hideFAB,
              final boolean allowContextualMode) {
  ...
  public View onCreateView(final LayoutInflater inflater,
    ...
    boolean allowContextualActions =
        (args != null) && args.getBoolean(ARG_ALLOW_CONTEXTUAL_MODE, false);
  ...
  public void onActivityCreated(final Bundle savedInstanceState) {
    ...
    if (mFileListOption == null && getArguments() != null &&
        getArguments().getParcelable(ARG_LIST_FILE_OPTION) != null) {
  ...
  public void updateFileListOption(final FileListOption newFileListOption) {
    updateListOfFiles(newFileListOption);
    mFileListOption = newFileListOption;
    mFile =
        mContainerActivity.getStorageManager().getFileByPath(OCFile.ROOT_PATH);
    listDirectory(true);
  }

  private void updateListOfFiles(final FileListOption fileListOption) {
    ...
    mFileListAdapter = new FileListListAdapter(
        justFolders, fileListOption.isAvailableOffline(),
        fileListOption.isSharedByLink(), folderPicker, getActivity(),
        mContainerActivity);
  ...
  private void setFabLabels() {
    getFabUpload().setTitle(
        getResources().getString(R.string.actionbar_upload));
  ...
  /**
   * registers {@link android.view.View.OnClickListener} and {@link
   * android.view.View.OnLongClickListener} on the Upload mini FAB for the
   * linked action an {@link Snackbar} showing the underlying action.
   */
  private void registerFabUploadListeners() {
    getFabUpload().setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(final View v) {
        final View uploadBottomSheet = getLayoutInflater().inflate(
            R.layout.upload_bottom_sheet_fragment, null);
        final BottomSheetDialog dialog =
            new BottomSheetDialog(requireContext());
        dialog.setContentView(uploadBottomSheet);
        final LinearLayout uploadFilesLinearLayout =
            uploadBottomSheet.findViewById(R.id.files_linear_layout);
        LinearLayout uploadFromCameraLinearLayout =
            uploadBottomSheet.findViewById(
                R.id.upload_from_camera_linear_layout);
        TextView uploadToTextView =
            uploadBottomSheet.findViewById(R.id.upload_to_text_view);
        uploadFilesLinearLayout.setOnTouchListener((v13, event) -> {
          Intent action = new Intent(Intent.ACTION_GET_CONTENT);
          action = action.setType(ALL_FILES_SAF_REGEX)
                       .addCategory(Intent.CATEGORY_OPENABLE);
          action.putExtra(Intent.EXTRA_ALLOW_MULTIPLE, true);
          getActivity().startActivityForResult(
              Intent.createChooser(action,
                                   getString(R.string.upload_chooser_title)),
        ...
        uploadFromCameraLinearLayout.setOnTouchListener((v12, event) -> {
          ((FileDisplayActivity)getActivity())
              .getFilesUploadHelper()
              .uploadFromCamera(
                  FileDisplayActivity.REQUEST_CODE__UPLOAD_FROM_CAMERA);
          dialog.hide();
          return false;
        });
        uploadToTextView.setText(
            String.format(getResources().getString(R.string.upload_to),
        ...
        dialog.setOnShowListener(dialog1
                                 -> uploadBottomSheetBehavior.setPeekHeight(
                                     uploadBottomSheet.getMeasuredHeight()));
        dialog.show();
        getFabMain().collapse();
        recordMiniFabClick();
      }
  ...
  /**
   * Registers {@link android.view.View.OnClickListener} and {@link
   * android.view.View.OnLongClickListener} on the 'Create Dir' mini FAB for the
   * linked action and {@link Snackbar} showing the underlying action.
   */
  ...
  private void removeFabLabels() {
    ...
    ((TextView)getFabUpload().getTag(
         com.getbase.floatingactionbutton.R.id.fab_label))
  ...
  public void onFocusChange(final View v, final boolean hasFocus) {
    if (hasFocus) {
      setMessageForEmptyList(
          getString(R.string.local_file_list_search_with_no_matches));
   ...
   * <p>
   * Manages input from the user when one or more files or folders are selected
   * in the list. <p> Also listens to changes in navigation drawer to hide and
   * recover multiple selection when it's opened and closed.
   */
  private class MultiChoiceModeListener
      implements AbsListView.MultiChoiceModeListener,
                 DrawerLayout.DrawerListener {

    private static final String KEY_ACTION_MODE_CLOSED_BY_DRAWER =
        "KILLED_ACTION_MODE";
    private static final String KEY_SELECTION_WHEN_CLOSED_BY_DRAWER =
        "CHECKED_ITEMS";
    ...
    @Override
    public void onDrawerSlide(final @NonNull View drawerView,
    ...
    /**
     * When the navigation drawer is closed, action mode is recovered in the
     * same state as was when the drawer was (started to be) opened.
     *
     * @param drawerView Navigation drawer just closed.
     */
    @Override
    public void onDrawerClosed(final @NonNull View drawerView) {
      if (mSelectionWhenActionModeClosedByDrawer != null &&
          mActionModeClosedByDrawer) {
        for (int i = 0; i < mSelectionWhenActionModeClosedByDrawer.size();
             i++) {
          if (mSelectionWhenActionModeClosedByDrawer.valueAt(i)) {
            getListView().setItemChecked(
                mSelectionWhenActionModeClosedByDrawer.keyAt(i), true);
          }
        }
      }
    ...
    /**
     * If the action mode is active when the navigation drawer starts to move,
     * the action mode is closed and the selection stored to be recovered when
    ...
    public void onDrawerStateChanged(final int newState) {
      if (DrawerLayout.STATE_DRAGGING == newState &&
        ...
        mSelectionWhenActionModeClosedByDrawer =
            getListView().getCheckedItemPositions().clone();
    ...
    public void onItemCheckedStateChanged(final ActionMode mode,
      ...
      mode.invalidate();
      if (mFileListAdapter.getCheckedItems(getListView()).size() ==
          mFileListAdapter.getCount()) {
    ...
    public boolean onPrepareActionMode(final ActionMode mode, final Menu menu) {
      List<OCFile> checkedFiles =
          mFileListAdapter.getCheckedItems(getListView());
      final int checkedCount = checkedFiles.size();
      String title = getResources().getQuantityString(
          R.plurals.items_selected_count, checkedCount, checkedCount);
      mode.setTitle(title);
      FileMenuFilter mf = new FileMenuFilter(
          checkedFiles, ((FileActivity)requireActivity()).getAccount(),
          mContainerActivity, getActivity());
      mf.filter(menu, mEnableSelectAll, true,
                mFileListOption.isAvailableOffline(),
    ...
    void storeStateIn(final Bundle outState) {
    ...
    void loadStateFrom(final Bundle savedInstanceState) {
      mActionModeClosedByDrawer = savedInstanceState.getBoolean(
          KEY_ACTION_MODE_CLOSED_BY_DRAWER, mActionModeClosedByDrawer);
      SparseBooleanArrayParcelable sbap =
          savedInstanceState.getParcelable(KEY_SELECTION_WHEN_CLOSED_BY_DRAWER);
      if (sbap != null) {
   ...
   */
  private void setChoiceModeAsMultipleModal(final Bundle savedInstanceState) {
  ...
  public void onSaveInstanceState(final Bundle outState) {
    ...
    // If this fragment is used to show target folders where a selected
    // file/folder can be copied/moved, multiple choice is disabled
  ...
  public void onPrepareOptionsMenu(final @NonNull Menu menu) {
    ...
    changeGridIcon(
        menu); // this is enough if the option stays out of the action bar
  }
   ...
   * existing folders. <p/> return       Count of folder levels browsed up.
   */
  ...
  private void listDirectoryWithAnimationDown(final OCFile file) {
    if (isInPowerSaveMode()) {
      ...
      Animation fadeOutFront =
          AnimationUtils.loadAnimation(getContext(), R.anim.dir_fadeout_front);
      Handler eventHandler = new Handler();

      // This is a ugly hack for getting rid of the "ArrayOutOfBound" exception
      // we get when we call listDirectory() from the Animation callback
  ...
  private void listDirectoryWidthAnimationUp(final OCFile file) {
    if (isInPowerSaveMode()) {
      ...
      Animation fadeOutBack =
          AnimationUtils.loadAnimation(getContext(), R.anim.dir_fadeout_back);
    ...
    if (file != null) {
      if (file.isFolder()) {
        ...
        if (PreviewImageFragment.canBePreviewed(file)) {
          ...
          // If the file is already downloaded sync it, just to update it if
          // there is a new available file version
          if (file.isDown()) {
            mContainerActivity.getFileOperationsHelper().syncFile(file);
          }
        } else {
   ...
   */
  private boolean fileIsDownloading(final OCFile file) {
  ...
  public int getNoOfItems() { return getListView().getCount(); }

  /**
   * Start the appropriate action(s) on the currently selected files given menu
   * selected by the user.
   *
   * @param menuId Identifier of the action menu selected by the user
   * @return 'true' if the menu selection started any action, 'false' otherwise.
   */
  private boolean onFileActionChosen(final int menuId) {
    final ArrayList<OCFile> checkedFiles =
        mFileListAdapter.getCheckedItems(getListView());
    ...
    if (checkedFiles.size() == 1) {
      ...
      case R.id.action_rename_file: {
        RenameFileDialogFragment dialog =
            RenameFileDialogFragment.newInstance(singleFile);
      ...
      case R.id.action_send_file: {
        ...
        if (!singleFile.isDown()) { // Download the file
          ...
          ((FileDisplayActivity)mContainerActivity)
              .startDownloadForSending(singleFile);
    ...
    case R.id.action_remove_file: {
      RemoveFilesDialogFragment dialog =
          RemoveFilesDialogFragment.newInstance(checkedFiles);
    ...
    case R.id.action_cancel_sync: {
      ((FileDisplayActivity)mContainerActivity)
          .cancelTransference(checkedFiles);
      return true;
    }
    ...
    case R.id.action_move: {
      ...
      action.putParcelableArrayListExtra(FolderPickerActivity.EXTRA_FILES,
                                         checkedFiles);
      requireActivity().startActivityForResult(
          action, FileDisplayActivity.REQUEST_CODE__MOVE_FILES);
      return true;
    }
    case R.id.action_copy:
      ...
      action.putParcelableArrayListExtra(FolderPickerActivity.EXTRA_FILES,
                                         checkedFiles);
      requireActivity().startActivityForResult(
          action, FileDisplayActivity.REQUEST_CODE__COPY_FILES);
      return true;
    default:
      return false;
    }
   ...
   */
  public OCFile getCurrentFile() { return mFile; }

  /**
   * Calls {@link OCFileListFragment#listDirectory(OCFile)} with a null
   * parameter
   */
  public void listDirectory(final boolean reloadData) {
    if (reloadData) {
      listDirectory(null);
    } else {
      getListView().invalidateViews();
   ...
   */
  public void listDirectory(final OCFile directory) {
    FileDataStorageManager storageManager =
        mContainerActivity.getStorageManager();
    if (storageManager != null) {
      ...
      if (!directory.isFolder()) {
        Timber.w("You see, that is not a directory -> %s",
                 directory.toString());
  ...
  private String generateFooterText(final int filesCount,
    ...
    if (filesCount <= 0) {
      ...
      if (foldersCount <= 0) {
        ...
        output = getResources().getString(
            R.string.file_list__footer__file_and_folder);

      } else { // foldersCount > 1
        output = getResources().getString(
            R.string.file_list__footer__file_and_folders, foldersCount);
      }
    } else { // filesCount > 1
      if (foldersCount <= 0) {
        ...
        output = getResources().getString(
            R.string.file_list__footer__files_and_folder, filesCount);

      } else { // foldersCount > 1
        output = getResources().getString(
            R.string.file_list__footer__files_and_folders, filesCount,
            foldersCount);
      }
    }
    return output;
  ...
  public void sortByName(final boolean descending) {
    mFileListAdapter.setSortOrder(FileStorageUtils.SORT_NAME, descending);
  }

  public void sortByDate(final boolean descending) {
    mFileListAdapter.setSortOrder(FileStorageUtils.SORT_DATE, descending);
  }

  public void sortBySize(final boolean descending) {
    mFileListAdapter.setSortOrder(FileStorageUtils.SORT_SIZE, descending);
  }

  /**
   * Determines if user set folder to grid or list view. If folder is not set
   * itself, it finds a parent that is set (at least root is set).
   *
   * @param file Folder to check.
   * @return 'true' is folder should be shown in grid mode, 'false' if list mode
   *     is preferred.
   */
  private boolean isGridViewPreferred(final OCFile file) {
    if (file != null) {
  ...
  private boolean endWhile(final OCFile parentDir,
  ...
  private void changeGridIcon(final Menu menu) {
