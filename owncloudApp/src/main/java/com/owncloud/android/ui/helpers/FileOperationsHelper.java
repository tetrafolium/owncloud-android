...
public class FileOperationsHelper {
  ...
  public void openFile(final OCFile file) {
    if (file != null) {
      ...
      intentForSavedMimeType.setDataAndType(
          file.getExposedFileUri(mFileActivity), file.getMimetype());
      ...
      if (storagePath.lastIndexOf('.') >= 0) {
        ...
        if (guessedMimeType != null &&
            !guessedMimeType.equals(file.getMimetype())) {
          intentForGuessedMimeType = new Intent(Intent.ACTION_VIEW);
          intentForGuessedMimeType.setDataAndType(
              file.getExposedFileUri(mFileActivity), guessedMimeType);
      ...
      List<ResolveInfo> launchables =
          mFileActivity.getPackageManager().queryIntentActivities(
              openFileWithIntent, PackageManager.MATCH_DEFAULT_ONLY);

      if (launchables != null && launchables.size() > 0) {
        try {
          mFileActivity.startActivity(Intent.createChooser(
              openFileWithIntent,
  ...
  /**
   * Show dialog to allow the user to choose an app to send the private link of
   * an {@link OCFile}, or copy it to clipboard.
   *
   * @param file @param file {@link OCFile} which will be shared with internal
   *     users
   */
  ...
  /**
   * Show dialog to allow the user to choose an app to send the link of an
   * {@link OCShare}, or copy it to clipboard.
   *
   * @param share {@link OCShare} which link will be sent to the app chosen by
   *     the user.
   */
  ...
  /**
   * Show an instance of {@link
   * com.owncloud.android.domain.sharing.shares.model.ShareType} for sharing or
   * unsharing the {@link OCFile} received as parameter.
   *
  ...
  public void sendDownloadedFile(final OCFile file) {
    if (file != null) {
      ...
      String[] packagesToExclude =
          new String[] {mFileActivity.getPackageName()};
      DialogFragment chooserDialog =
          ShareLinkToDialog.newInstance(sendIntent, packagesToExclude);
  ...
  /**
   * Request the synchronization of a file or folder with the OC server,
  ...
  public void syncFile(final OCFile file) {
    if (!file.isFolder()) {
      ...
      intent.putExtra(OperationsService.EXTRA_ACCOUNT,
      ...
      intent.putExtra(OperationsService.EXTRA_REMOTE_PATH,
                      file.getRemotePath());
      intent.putExtra(OperationsService.EXTRA_SYNC_REGULAR_FILES, true);
      mFileActivity.startService(intent);
    }
  }

  public void toggleAvailableOffline(final Collection<OCFile> files,
  ...
  public void toggleAvailableOffline(final OCFile file,
                                     final boolean isAvailableOffline) {
    if (OCFile.AvailableOfflineStatus.AVAILABLE_OFFLINE_PARENT ==
        file.getAvailableOfflineStatus()) {
      ...
      OCFile.AvailableOfflineStatus targetAvailableOfflineStatus =
          isAvailableOffline
      ...
      boolean success =
          mFileActivity.getStorageManager().saveLocalAvailableOfflineStatus(
              file);

      if (success) {
        // Schedule job to check to watch for local changes in available offline
        // files and sync them
        AvailableOfflineHandler availableOfflineHandler =
            new AvailableOfflineHandler(mFileActivity,
                                        mFileActivity.getAccount().name);
        availableOfflineHandler.scheduleAvailableOfflineJob(mFileActivity);

        /// immediate content synchronization
        if (OCFile.AvailableOfflineStatus.AVAILABLE_OFFLINE ==
            file.getAvailableOfflineStatus()) {
          syncFile(file);
        } else {
  ...
  public void renameFile(final OCFile file, final String newFilename) {
    ...
    service.putExtra(OperationsService.EXTRA_ACCOUNT,
                     mFileActivity.getAccount());
    service.putExtra(OperationsService.EXTRA_REMOTE_PATH, file.getRemotePath());
    service.putExtra(OperationsService.EXTRA_NEWNAME, newFilename);
    mWaitingForOpId =
        mFileActivity.getOperationsServiceBinder().queueNewOperation(service);
   ...
   * @param onlyLocalCopy When 'true' only local copy of the files is removed;
   *     otherwise files are also deleted
   *                      in the server.
   */
  public void removeFiles(final Collection<OCFile> files,
    ...
    for (OCFile file : files) {
      ...
      service.putExtra(OperationsService.EXTRA_ACCOUNT,
                       mFileActivity.getAccount());
      service.putExtra(OperationsService.EXTRA_REMOTE_PATH,
                       file.getRemotePath());
      service.putExtra(OperationsService.EXTRA_REMOVE_ONLY_LOCAL,
                       onlyLocalCopy);
      if (countOfFilesToRemove == files.size()) {
        isLastFileToRemove = true;
      }
      service.putExtra(OperationsService.EXTRA_IS_LAST_FILE_TO_REMOVE,
                       isLastFileToRemove);
      mWaitingForOpId =
          mFileActivity.getOperationsServiceBinder().queueNewOperation(service);
    }
  ...
  public void createFolder(final String remotePath,
  ...
  public void cancelTransference(final OCFile file) {
    ...
    FileDownloaderBinder downloaderBinder =
        mFileActivity.getFileDownloaderBinder();
   ...
   */
  public void moveFiles(final Collection<OCFile> files,
                        final OCFile targetFolder) {
    for (OCFile file : files) {
      Intent service = new Intent(mFileActivity, OperationsService.class);
      service.setAction(OperationsService.ACTION_MOVE_FILE);
      service.putExtra(OperationsService.EXTRA_NEW_PARENT_PATH,
   ...
   */
  public void copyFiles(final Collection<OCFile> files,
                        final OCFile targetFolder) {
    for (OCFile file : files) {
      Intent service = new Intent(mFileActivity, OperationsService.class);
  ...
  public long getOpIdWaitingFor() { return mWaitingForOpId; }
  ...
  private void shareLink(final String link) {
    ...
    if (username != null) {
      intentToShareLink.putExtra(
          ...
          mFileActivity.getString(R.string.subject_user_shared_with_you,
                                  username,
                                  mFileActivity.getFile().getFileName()));
    } else {
      intentToShareLink.putExtra(
          Intent.EXTRA_SUBJECT,
          mFileActivity.getString(R.string.subject_shared_with_you,
                                  mFileActivity.getFile().getFileName()));
    }

    String[] packagesToExclude = new String[] {mFileActivity.getPackageName()};
    DialogFragment chooserDialog =
        ShareLinkToDialog.newInstance(intentToShareLink, packagesToExclude);
    chooserDialog.show(mFileActivity.getSupportFragmentManager(),
                       FTAG_CHOOSER_DIALOG);
  }
}
