...
void onCheckAvailableSpaceFinished(boolean hasEnoughSpace,
                                   String[] capturedFilePaths);
}

/**
 * Asynchronous task checking if there is space enough to copy all the files
 * chosen to upload into the ownCloud local folder. <p> Maybe an AsyncTask is
 * not strictly necessary, but who really knows.
 */
...
public CheckAvailableSpaceTask(final String[] checkedFilePaths,
                               final OnCheckAvailableSpaceListener
                               listener) {
	super();
...
protected Boolean doInBackground(final Void... params) {
	long total = 0;
	for (int i = 0; checkedFilePaths != null && i < checkedFilePaths.length;
	     i++) {
 ...
 *
 * @param hasEnoughSpace 'True' when there is space enough to copy all the
 *     selected files.
 */
@Override
...
public void
checkIfAvailableSpace(final String[] checkedFilePaths,
...
public static String getCapturedImageName() {
	...
	minute =
		minute.length() == 3 ? minute.substring(1, minute.length()) : minute;
	second =
		second.length() == 3 ? second.substring(1, second.length()) : second;
	String newImageName =
		"IMG_" + year + month + day + "_" + hour + minute + second;
	return newImageName;
}
...
public void uploadFromCamera(final int requestCode) {
	...
	if (photoFile != null) {
		Uri photoUri = FileProvider.getUriForFile(
			...
			activity.getResources().getString(R.string.file_provider_authority),
			photoFile);
...
public void onActivityResult(final OnCheckAvailableSpaceListener callback) {
	checkIfAvailableSpace(
		new String[] {getCapturedImageFile().getAbsolutePath()}, callback);
}
...
public static final Parcelable.Creator<FilesUploadHelper> CREATOR =
	new Parcelable.Creator<FilesUploadHelper>() {
	@Override
