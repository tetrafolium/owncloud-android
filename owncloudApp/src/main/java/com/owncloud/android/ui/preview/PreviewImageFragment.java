 ...
 * <p>
 * If the {@link OCFile} passed is not downloaded, an {@link
 * IllegalStateException} is generated on instantiation too.
 */
 ...
 * needed arguments. <p> This method hides to client objects the need of doing
 * the construction in two steps.
 *
 * @param file                  An {@link OCFile} to preview as an image in
 *     the fragment
 ...
 */
public static PreviewImageFragment
...
public View onCreateView(final LayoutInflater inflater,
	...
	View view =
		inflater.inflate(R.layout.preview_image_fragment, container, false);
...
public void onActivityCreated(final Bundle savedInstanceState) {
	...
	if (savedInstanceState != null) {
		if (!mIgnoreFirstSavedState) {
			OCFile file =
				savedInstanceState.getParcelable(PreviewImageFragment.EXTRA_FILE);
			setFile(file);
		} else {
			mIgnoreFirstSavedState = false;
		}
...
public void onPrepareOptionsMenu(final Menu menu) {
	...
	if (mContainerActivity.getStorageManager() != null && getFile() != null) {
		// Update the file
		setFile(mContainerActivity.getStorageManager().getFileById(
				getFile().getFileId()));
...
public void onDestroy() {
	if (mBitmap != null) {
		mBitmap.recycle();
		System.gc();
		// putting this in onStop() is just the same; the fragment is always
		// destroyed by
		// {@link FragmentStatePagerAdapter} when the fragment in swiped further
		// than the valid offscreen distance, and onStop() is never called before
		// than that
	}
	super.onDestroy();
}
...
private void loadAndShowImage() {
	...
	.listener(new RequestListener<Drawable>() {
			...
			public boolean onLoadFailed(
				final @Nullable GlideException e, final Object model,
 ...
 * PreviewImageFragment} to be previewed.
 *
