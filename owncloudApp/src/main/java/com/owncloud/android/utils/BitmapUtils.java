...
public class BitmapUtils {
  ...
  /**
   * Decodes a bitmap from a file containing it minimizing the memory use, known
   * that the bitmap will be drawn in a surface of reqWidth x reqHeight
   *
   * @param srcPath       Absolute path to the file containing the image.
   * @param reqWidth      Width of the surface where the Bitmap will be drawn
   *     on, in pixels.
   * @param reqHeight     Height of the surface where the Bitmap will be drawn
   *     on, in pixels.
   * @return
   */
  public static Bitmap decodeSampledBitmapFromFile(final String srcPath,
                                                   final int reqWidth,
                                                   final int reqHeight) {
  ...
  /**
   * Calculates a proper value for options.inSampleSize in order to decode a
   * Bitmap minimizing the memory overload and covering a target surface of
   * reqWidth x reqHeight if the original image is big enough.
   *
   * @param options       Bitmap decoding options; options.outHeight and
   *     options.inHeight should
   *                      be set.
   * @param reqWidth      Width of the surface where the Bitmap will be drawn
   *     on, in pixels.
   * @param reqHeight     Height of the surface where the Bitmap will be drawn
   ...
   */
  private static int calculateSampleFactor(final Options options,
                                           final int reqWidth,
                                           final int reqHeight) {
    ...
    if (height > reqHeight || width > reqWidth) {
      ...
      // calculates the largest inSampleSize value (for smallest sample) that is
      // a power of 2 and keeps both height and width **larger** than the
  ...
  private static float fixRawHSLValue(final float value, final float upperBound,
                                      final float scale) {
    return (value > upperBound) ? upperBound
                                : (value < 0f) ? 0f : value * scale;
  }
  ...
  public static int[] HSLtoRGB(final float h, final float s, final float l,
    ...
    final int r =
        Math.round(Math.max(0, HueToRGB(p, q, hr + (1.0f / 3.0f)) * 256));
  ...
  private static float HueToRGB(final float p, final float q, final float h) {
    final float hr = (h < 0) ? h + 1 : (h > 1) ? h - 1 : h;
  ...
  public static boolean isImage(final File file) {
    ...
    final String fileExtension = MimeTypeMap.getFileExtensionFromUrl(
        selectedUri.toString().toLowerCase());
  ...
  public static int[] calculateAvatarBackgroundRGB(final String accountName)
    ...
    // Integer seedMd5Int = Math.abs(new
    // String(Hex.encodeHex(seedMd5)).hashCode());
    final Integer seedMd5Int =
        String.format(Locale.ROOT, "%032x", new BigInteger(1, md.digest(seed)))
            .hashCode();
  ...
  /**
   * Returns a new circular bitmap drawable by creating it from a bitmap,
   * setting initial target density based on the display metrics of the
   * resources.
   *
  ...
  bitmapToCircularBitmapDrawable(final Resources resources,
                                 final Bitmap bitmap) {
