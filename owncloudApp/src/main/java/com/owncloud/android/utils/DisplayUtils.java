...
private static final String[] sizeSuffixes = {"B",  "KB", "MB", "GB", "TB",
	                                      "PB", "EB", "ZB", "YB"};
...
public static String bytesToHumanReadable(final long bytes,
	...
	if (bytes < 0) {
		...
		BigDecimal readableResult =
			new BigDecimal(result)
			.setScale(sizeScales[attachedSuff], BigDecimal.ROUND_HALF_UP)
			.stripTrailingZeros();

		// Unscale only values with ten exponent
		return (readableResult.scale() < 0 ? readableResult.setScale(0)
		                         : readableResult) +
...
/**
 * Converts an internationalized domain name (IDN) in an URL to and from
 * ASCII/Unicode.
 *
 * @param url     the URL where the domain name should be converted
 * @param toASCII if true converts from Unicode to ASCII, if false converts
...
public static String convertIdn(final String url, final boolean toASCII) {
	...
	return dots + urlNoDots.substring(0, hostStart) + host +
	       urlNoDots.substring(hostEnd);
}

/**
 * calculates the relative time string based on the given modificaion
 * timestamp.
 *
 * @param context               the app's context
 * @param modificationTimestamp the UNIX timestamp of the file modification
 *     time.
 * @return a relative time string
 */
public static CharSequence
getRelativeTimestamp(final Context context,
                     final long modificationTimestamp) {
	return getRelativeDateTimeString(context, modificationTimestamp,
	                                 DateUtils.SECOND_IN_MILLIS,
	                                 DateUtils.WEEK_IN_MILLIS, 0);
}

public static CharSequence
getRelativeDateTimeString(final Context c, final long time,
                          final long minResolution,
                          final long transitionResolution, final int flags) {
	...
	if (time > System.currentTimeMillis()) {
		...
		dateString = DateUtils.getRelativeDateTimeString(
			c, time, minResolution, transitionResolution, flags);
	}
...
public static String getPathWithoutLastSlash(final String path) {
	...
	if (path.length() > 1 &&
	    path.charAt(path.length() - 1) == OCFile.PATH_SEPARATOR.charAt(0)) {
 ...
 *
 * @param caller Activity calling; needed to get access to the {@link
 *     android.view.WindowManager}
 * @return Size in pixels of the screen, or default {@link Point} if caller is
 *     null
 */
 ...
 *
 * @param context  the context relevant for setting the color according to the
 *     context's theme
 * @param snackbar the snackbar to be colored
 */
public static void colorSnackbar(final Context context,
 ...
 * <p>
 * The avatar is shown if available locally in {@link ThumbnailsCacheManager}.
 * The avatar is not fetched from the server if not available. <p> If there is
 * no avatar stored, a colored icon is generated with the first letter of the
 * account username. <p> If this is not possible either, a predefined user
 ...
 */
public static void showAccountAvatar(final Account account,
                                     final ImageView displayView,
                                     final float displayRadius,
                                     final boolean fetchFromServer) {
	if (account != null) {
		// not just accessibility support, used to know what account is bound to
		// each imageView
