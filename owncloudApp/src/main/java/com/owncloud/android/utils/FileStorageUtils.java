...
public class FileStorageUtils {
   ...
   * file.
   */
  public static String getDefaultSavePathFor(final String accountName,
                                             final OCFile file) {
    return getSavePath(accountName) + file.getRemotePath();
  }

  /**
   * Get absolute path to tmp folder inside datafolder in sd-card for given
   * accountName.
   */
  public static String getTemporalPath(final String accountName) {
    File sdCard = Environment.getExternalStorageDirectory();
    return sdCard.getAbsolutePath() + "/" + MainApp.Companion.getDataFolder() +
        "/tmp/" + Uri.encode(accountName, "@");
    // URL encoding is an 'easy fix' to overcome that NTFS and FAT32 don't allow
    // ":" in file names, that can be in the accountName since 0.1.190B
  }
   ...
   */
  public static Vector<OCFile> sortFolder(final Vector<OCFile> files,
                                          final int sortOrder,
   ...
   * Sorts list by Date
   */
  private static void sortByDate(final Vector<OCFile> files,
                                 final boolean isAscending) {
    final int val;
    if (isAscending) {
      val = 1;
    ...
    Collections.sort(files, (ocFile1, ocFile2) -> {
      if (ocFile1.getModificationTimestamp() == 0 ||
          ocFile2.getModificationTimestamp() == 0) {
        return 0;
      } else {
   ...
   * Sorts list by Size
   */
  private static void sortBySize(final Vector<OCFile> files,
                                 final boolean isAscending) {
    final int val;
    if (isAscending) {
      val = 1;
   ...
   */
  private static void sortByName(final Vector<OCFile> files,
                                 final boolean isAscending) {
    final int val;
    if (isAscending) {
      val = 1;
    ...
    Collections.sort(files, (ocFile1, ocFile2) -> {
      if (ocFile1.isFolder() && ocFile2.isFolder()) {
        return val * ocFile1.getFileName().toLowerCase().compareTo(
                         ocFile2.getFileName().toLowerCase());
